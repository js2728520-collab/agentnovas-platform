...
export function InternalAccountSecurity() {
  const status = useApiData<RecoveryStatus>("/api/auth/mfa/recovery-codes", "双重验证状态读取失败");
  ...
  async function submit(reason: string) {
    ...
    try {
      if (dialog === "rotate") {
        ...
        if (!response.ok) throw new Error(apiErrorMessage(payload, "恢复码轮换失败"));
        setRecoveryCodes(Array.isArray(payload.recoveryCodes) ? payload.recoveryCodes : []);
        announce("恢复码已轮换。旧的未使用恢复码已经失效。");
      ...
      announce(error instanceof Error ? error.message : "操作失败");
    } finally {
      setBusy(false);
    }
  }
  ...
  return <>
    ...
    <section className="rc-panel">
      ...
      {recoveryCodes ? <div className="rc-warning" role="alert">
        ...
        <p>请立即保存到受控密码管理器。关闭或刷新页面后无法再次查看。</p>
        <ol>{recoveryCodes.map((code) => <li key={code}><code>{code}</code></li>)}</ol>
        <button className="rc-button" type="button" onClick={() => setRecoveryCodes(null)}>我已安全保存并关闭</button>
      </div> : null}
    </section>
    <section className="rc-panel">
      <header><div><small>ACTIVE SESSIONS</small><h2>内部登录设备</h2></div><button className="rc-button" type="button" onClick={() => void sessions.refresh()}>重新读取</button></header>
      {sessions.loading && !sessions.data ? <LoadingState label="正在读取登录设备…" /> : sessions.error && !sessions.data ? <ErrorState message={sessions.error} retry={sessions.refresh} /> : <div className="rc-card-grid">
        {sessions.data?.sessions.map((session) => <article className="rc-card" key={session.id}>
          <header><StatusBadge value={session.current ? "当前设备" : session.audience} /><time>{formatDateTime(session.lastSeenAt ?? session.createdAt)}</time></header>
          <h3>{session.device}</h3><p>{session.maskedIpAddress ?? "未记录网络地址"}</p>
          <footer>{session.current ? <span className="rc-muted">当前设备请使用账户菜单退出。</span> : <button className="rc-button" type="button" disabled={busy} onClick={() => setDialog(session)}>撤销此设备</button>}</footer>
        </article>)}
      </div>}
    </section>
