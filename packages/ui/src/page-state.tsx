...
export function LoadingState({ label = "正在读取数据…" }: { label?: string }) {
  return <div className="rc-state rc-loading" aria-busy="true" aria-label={label}><i /><i /><i /><span>{label}</span></div>;
}
...
export function ErrorState({ message, retry }: { message: string; retry?: () => void }) {
  return <section className="rc-state rc-error" role="alert"><strong>读取失败</strong><p>{message}</p>{retry && <button type="button" onClick={retry}>重新读取</button>}</section>;
}

export function AccessDenied({ message = "当前账户没有访问此模块的权限。" }: { message?: string }) {
  return <section className="rc-state rc-denied" role="alert"><strong>无权访问</strong><p>{message}</p><Link href="/">返回工作台</Link></section>;
}

export function StatusBadge({ value }: { value: string | null | undefined }) {
  const normalized = String(value || "unknown").toLowerCase().replace(/[^a-z0-9_-]/g, "-");
  return <span className={`rc-status is-${normalized}`}>{value || "未知"}</span>;
}
