...
function AccessManagement({ appId, permissions }: { appId: Exclude<AppAudience, "client">; permissions: Record<string, DataScope> }) {
  ...
  async function execute(action: DirectAction | PendingAction) {
    setBusy(true); setMessage("");
    try {
      if (action.kind === "decision") {
        await jsonRequest(`/api/access/change-requests/${action.request.id}/decisions`, "POST", { decision: action.decision });
        ...
        if (selectedDefinitions.some((definition) => definition.sensitive)) {
          await jsonRequest("/api/access/change-requests", "POST", { applicationId: appId, changeType: "role_create", targetUserId: null, targetRoleId: null, before: {}, after: { code: code.trim(), name: name.trim(), permissions } });
          ...
          await jsonRequest(`/api/access/roles/${role.id}/publish`, "POST", {});
        ...
        if (selectedTemplateDefinitions.some((definition) => definition.sensitive)) {
          await jsonRequest("/api/access/change-requests", "POST", { applicationId: appId, changeType: "template_publish", targetUserId: null, targetRoleId: null, before: {}, after: { code: templateCode.trim(), name: templateName.trim(), permissions: templateRolePermissions, changeSummary } });
        ...
        await jsonRequest(`/api/access/roles/${action.role.id}/publish`, "POST", {});
        ...
        if (sensitive) {
          await jsonRequest("/api/access/change-requests", "POST", { applicationId: appId, changeType: "role_assign", targetUserId: targetUserId.trim(), targetRoleId: selectedRole.id, before: {}, after: { expiresAt: null } });
          setMessage(t("敏感角色分配申请已提交，等待第二人审批。"));
        } else {
          await jsonRequest("/api/access/assignments", "POST", { userId: targetUserId.trim(), roleId: selectedRole.id });
          setMessage(t("普通角色已分配。"));
        }
        setTargetUserId(""); setTargetRoleId("");
      } else {
        const assignment = action.assignment;
        await jsonRequest("/api/access/change-requests", "POST", { applicationId: appId, changeType: "role_revoke", targetUserId: assignment.userId, targetRoleId: assignment.roleId, before: {}, after: { assignmentId: assignment.id } });
