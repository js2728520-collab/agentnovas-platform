...
export function EmailServiceTests({ status,tests,recipients,busy,translate = value=>value,formatDateTime,onSendTest,onRefresh }: Props) {
  ...
  const sendDisabled=busy || !recipient || status.effectiveStatus==="unconfigured" || status.effectiveStatus==="disabled" || !hasValidAuditReason(reason);
  async function send() {
    if (!recipient) return;
    if (!await onSendTest(recipient.id,reason.trim())) return;
    setReason("");
  }
  return <>
