...
export function EmailServiceConfiguration(props: Props) {
  ...
  async function submitSecrets() {
    if (!await props.onSecretChange({ operation: secretOperation,apiKey,webhookSecret,reason: secretReason.trim() })) return;
    setApiKey("");setWebhookSecret("");setSecretReason("");
  }

  async function createRecipient() {
    if (!await props.onRecipientCreate({ email: email.trim(),label: label.trim(),reason: createReason.trim() })) return;
    setEmail("");setLabel("");setCreateReason("");
  }
  ...
  return <>
    <section className="rc-panel"><header><div><small>PROVIDER & SECRET BROKER</small><h2>{t("Provider 与密钥")}</h2><p>{t("浏览器使用 Broker 公钥加密；Web、数据库和审计只接触密文，密钥不会回显。")}</p></div><StatusBadge value={status.secretManagement?.browserConfigurable ? "ready" : "unconfigured"} label={emailServiceStatusLabel(status.secretManagement?.browserConfigurable ? "ready" : "unconfigured")} /></header>
      ...
      {canManage ? <div className="rc-form">
        ...
        <InlineAuditReasonField id="email-secret-reason" value={secretReason} onChange={setSecretReason} label={t("密钥变更原因")} />
        <button className="rc-primary" type="button" disabled={busy || secretRequestActive || !status.secretManagement?.browserConfigurable || !validSecrets || !hasValidAuditReason(secretReason)} onClick={()=>void submitSecrets()}>{busy || secretRequestActive ? t("正在处理…") : t(secretOperation === "install" ? "加密提交配置" : "加密提交轮换")}</button>
      </div> : null}
    </section>
    ...
    <section className="rc-panel"><header><div><small>VERIFIED TEST RECIPIENTS</small><h2>{t("测试收件人")}</h2><p>{t("测试地址独立于管理员账号；必须通过邮箱验证码后才能用于投递测试。")}</p></div><span>{recipients.length} {t("个")}</span></header>
      {canManage ? <div className="rc-form">
        ...
        <InlineAuditReasonField id="email-recipient-create-reason" value={createReason} onChange={setCreateReason} label={t("新增原因")} />
        <button className="rc-primary" type="button" disabled={busy || !email.includes("@") || !label.trim() || !hasValidAuditReason(createReason)} onClick={()=>void createRecipient()}>{t("新增并发送验证码")}</button>
      </div> : null}
      {!recipients.length ? <EmptyState title={t("尚无测试收件人")} description={t("新增邮箱并完成验证码验证后，才能发送测试邮件。")}/> : <div className="rc-card-list rc-email-recipient-list">{recipients.map(recipient=><article key={recipient.id}>
        ...
        {canManage ? <div className="rc-action-row">
          {recipient.status === "pending_verification" ? <><button className="rc-primary" type="button" disabled={busy || (verificationCodes[recipient.id]?.length ?? 0)!==6 || !hasValidAuditReason(recipientReason)} onClick={()=>void props.onRecipientVerify({ recipientId: recipient.id,code: verificationCodes[recipient.id],reason: recipientReason.trim() })}>{t("验证")}</button><button className="rc-button" type="button" disabled={busy || !hasValidAuditReason(recipientReason)} onClick={()=>void props.onRecipientResend({ recipientId: recipient.id,reason: recipientReason.trim() })}>{t("重发验证码")}</button></> : <button className="rc-button" type="button" disabled={busy || !hasValidAuditReason(recipientReason)} onClick={()=>void props.onRecipientChange({ recipientId: recipient.id,action: recipient.status === "active" ? "disable" : "enable",reason: recipientReason.trim() })}>{t(recipient.status === "active" ? "停用" : "启用")}</button>}
          <button className="rc-button rc-danger-button" type="button" disabled={busy || !hasValidAuditReason(recipientReason)} onClick={()=>void props.onRecipientDelete({ recipientId: recipient.id,reason: recipientReason.trim() })}>{t("删除")}</button>
        </div> : null}
      </article>)}</div>}
      {canManage && recipients.length ? <InlineAuditReasonField id="email-recipient-action-reason" value={recipientReason} onChange={setRecipientReason} label={t("收件人操作原因")} /> : null}
    </section>
    ...
    <section className="rc-panel"><header><div><small>OUTBOUND AUTHORIZATION</small><h2>{t("Provider 外发授权")}</h2><p>{t("密钥和验证事实就绪后，再开启外发；关闭不会删除任何密钥。")}</p></div><StatusBadge value={status.providerAuthorized ? "active" : "disabled"} label={emailServiceStatusLabel(status.providerAuthorized ? "active" : "disabled")} /></header>
      {canManage ? <div className="rc-form"><InlineAuditReasonField id="email-provider-reason" value={providerReason} onChange={setProviderReason} label={t("授权变更原因")} /><button className={status.providerAuthorized ? "rc-button rc-danger-button" : "rc-primary"} type="button" disabled={busy || !hasValidAuditReason(providerReason)} onClick={()=>void props.onConfigurationChange(status.providerAuthorized ? "disable" : "activate",providerReason.trim())}>{t(status.providerAuthorized ? "关闭 Provider 外发" : "启用 Provider 外发")}</button></div> : null}
    </section>
  </>;
}
