...
test("seven-current-document gate, USD snapshot and bound idempotency activate safely",async()=>{
  ...
  assert.equal((await recordMembershipPaymentEvidence(pool,{orderId:order.id,actorUserId:"maker",evidenceKind:"bank_transfer",reference:"MAKER-SECRET-5678",amount:"28",currency:"USD",occurredAt:"2026-08-20T00:00:00Z",idempotencyKey:"order-evidence-maker"})).membership_order_id,order.id);
...
test("credits settle derives cost from trusted usage and rolls back every partial mutation",async()=>{
  ...
  await assert.rejects(settleAiCreditReservation(pool,{reservationId,idempotencyKey:"settle-1",requestId:"changed-usage",costModelVersion:"token-cost-v1",trustedUsage:{source:"provider_metering",usageId:"usage-1",inputTokens:0,outputTokens:1_000_000}}),/上下文不一致/);
  ...
  const collisionClient=await pool.connect();try{await collisionClient.query("BEGIN");await assert.rejects(reserveAiCredits(collisionClient,{userId:"customer2",credits:BigInt(100),sourceType:"inference",sourceId:"call-other",idempotencyKey:"reserve-1",requestId:"collision",expiresAt:"2026-08-21"}),/已绑定其他变更/);await collisionClient.query("ROLLBACK");}finally{collisionClient.release();}
...
test("posted ledger rejects every later posting",async()=>{
  ...
  const collisionClient=await pool.connect();try{await collisionClient.query("BEGIN");const platform=await ensurePlatformLedgerAccount(collisionClient,"platform_deposit_clearing","USDT");await assert.rejects(postCommercialLedgerTransaction(collisionClient,{transactionType:"correction",sourceType:"test",sourceId:"wallet-credit",currency:"USDT",idempotencyKey:"wallet-credit",requestId:"collision",createdByUserId:"checker",postings:[{accountId:platform,side:"debit",amount:"6"},{accountId:"customer-available",side:"credit",amount:"6"}],walletMutation:{userId:"customer",availableDelta:"6",frozenDelta:"0"}}),/已绑定其他交易/);await collisionClient.query("ROLLBACK");}finally{collisionClient.release();}
});
...
test("membership rejection rolls back decision, audit and outbox as one unit",async()=>{
  const order=await readyOrder("membership_monthly_v1","reject-atomic",{userId:"customer2"});
  ...
  await assert.rejects(decideMembershipOrder(pool,{orderId:order.id,reviewerUserId:"checker",decision:"reject",note:"reject",idempotencyKey:"reject-atomic-decision",requestId:"reject-atomic"}),/forced reject outbox failure/);
  assert.equal((await pool.query(`SELECT status FROM commercial_membership_orders WHERE id=$1`,[order.id])).rows[0].status,"pending_review");
  assert.equal((await pool.query(`SELECT count(*)::int count FROM commercial_membership_order_decisions WHERE order_id=$1`,[order.id])).rows[0].count,0);
  assert.equal((await pool.query(`SELECT count(*)::int count FROM audit_logs WHERE subject_id=$1`,[order.id])).rows[0].count,0);
  await pool.query(`DROP TRIGGER fail_membership_reject_outbox ON notification_deliveries; DROP FUNCTION fail_membership_reject_outbox()`);
  assert.equal((await decideMembershipOrder(pool,{orderId:order.id,reviewerUserId:"checker",decision:"reject",note:"reject",idempotencyKey:"reject-atomic-decision",requestId:"reject-atomic"})).status,"rejected");
});
...
test("only the previous complete UTC week settles server-resolved scope with HWM sequencing",async()=>{
  ...
  await pool.query(`CREATE OR REPLACE FUNCTION fail_hwm_commit() RETURNS trigger AS $$ BEGIN RAISE EXCEPTION 'forced hwm failure'; END $$ LANGUAGE plpgsql; CREATE TRIGGER fail_hwm_commit BEFORE UPDATE ON performance_fee_high_water_marks FOR EACH ROW EXECUTE FUNCTION fail_hwm_commit();`);
  await assert.rejects(decidePerformancePayment(pool,{statementId:statement.id,reviewerUserId:"checker2",decision:"approve",note:"verified",idempotencyKey:"payment-approved",requestId:"payment-approved"}),/forced hwm failure/);
  assert.equal((await pool.query(`SELECT status,ledger_transaction_id FROM performance_fee_statements WHERE id=$1`,[statement.id])).rows[0].status,"payment_pending");
  assert.equal((await pool.query(`SELECT count(*)::int count FROM ledger_transactions WHERE source_type='performance_fee_statement' AND source_id=$1`,[statement.id])).rows[0].count,0);
  await pool.query(`DROP TRIGGER fail_hwm_commit ON performance_fee_high_water_marks; DROP FUNCTION fail_hwm_commit()`);
  const paid=await decidePerformancePayment(pool,{statementId:statement.id,reviewerUserId:"checker2",decision:"approve",note:"verified",idempotencyKey:"payment-approved",requestId:"payment-approved"});assert.equal(paid.status,"paid");const paidLedger=(await pool.query(`SELECT lt.id,lt.transaction_type FROM performance_fee_statements s JOIN ledger_transactions lt ON lt.id=s.ledger_transaction_id WHERE s.id=$1`,[statement.id])).rows[0];assert.equal(paidLedger.transaction_type,"performance_fee_payment");
  assert.equal((await recordPerformancePaymentEvidence(pool,{statementId:statement.id,actorUserId:"maker",evidenceKind:"bank_transfer",reference:"PAYMENT-MAKER-5678",amount:"40",currency:"USDT",occurredAt:"2026-08-20T00:00:00Z",idempotencyKey:"payment-evidence-maker"})).amount,"40.000000000000000000");
  for(const action of ["commercial.performance.generated","commercial.performance.assessment.approve","commercial.performance.payment_evidence","commercial.performance.paid"])
    assert.ok((await pool.query(`SELECT count(*)::int count FROM audit_logs WHERE subject_id=$1 AND action=$2`,[statement.id,action])).rows[0].count>=1,`missing audit ${action}`);
