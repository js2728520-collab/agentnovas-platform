...
import { expectedWebDatabaseRole, isolatedQualityDatabaseRoleBypass } from "../lib/postgres.ts";
...
test("Client identity RLS and capability gateways partition internal and cross-customer identity", async () => {
  ...
  assert.match(migration, /current_user\s*=\s*'agentnovas_client_web'/i);
  assert.match(migration, /current_user IN \('agentnovas_migrator','agentnovas_ops_web','agentnovas_maint_web'/i);
  assert.doesNotMatch(migration, /current_setting|set_config|request\.|jwt|session_user/i);
  ...
  assert.match(migration, /SET search_path FROM CURRENT/i);
...
test("Web pools bind audience roles and only allow the isolated loopback quality schema exception", async () => {
  ...
  assert.match(source, /expectedRole \? businessDatabaseUrl\(\) : researchDatabaseUrl\(\)/);
