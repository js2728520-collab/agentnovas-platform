...
test("Operations Credits UI exposes maker-checker controls without claiming submission changed balance", async () => {
  ...
  const submit = await readFile(new URL("../app/api/operations/credit-adjustments/route.operations.ts", import.meta.url), "utf8");
  const decision = await readFile(new URL("../app/api/operations/credit-adjustments/[id]/decision/route.operations.ts", import.meta.url), "utf8");
