...
test("maintenance model and integration workspaces are responsive, accessible and audience-isolated", async ({ page }) => {
  ...
  await expect(page.getByText(/审计原因|本轮配置原因/)).toHaveCount(0);
  await page.getByLabel("连接名称").fill("quality-browser-connection");
  await page.getByLabel("公共 HTTPS 基础地址").fill("https://api.example.com/v1");
  await page.getByLabel("部署名称").fill("quality-browser-deployment");
  await page.getByLabel("模型 ID").fill("quality-browser-model");
  await page.getByLabel(/API Key/).fill("quality-browser-placeholder-key");
  await expect(page.getByRole("button", { name: "保存并托管密钥" })).toBeEnabled();
});
...
test("maintenance configuration and controls submit inline without confirmation dialogs", async ({ page }) => {
  ...
  expect(testedResponse.request().postDataJSON()).toEqual({});
  ...
  await page.getByLabel("事故或处置说明（业务字段）").fill("浏览器验证内联紧急控制流程");
  ...
  if (!initiallyPaused) {
    ...
    await page.getByLabel("事故或处置说明（业务字段）").fill("浏览器验证结束后恢复初始状态");
