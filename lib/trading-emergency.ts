...
export async function isCustomerTradingEmergencyStopped(customerId: string) {
  ...
  const attribution = (await db.select({ branchId: customerAttributions.branchId }).from(customerAttributions).where(and(eq(customerAttributions.customerId, customerId), eq(customerAttributions.status, "active"))).orderBy(desc(customerAttributions.effectiveAt)).limit(1))[0];
  ...
  const states = await db.select({ scopeKey: tradingEmergencyStops.scopeKey }).from(tradingEmergencyStops).where(and(eq(tradingEmergencyStops.active, true), inArray(tradingEmergencyStops.scopeKey, scopeKeys)));
