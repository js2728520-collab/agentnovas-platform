...
export type CommercialApprovalCommand=
  ...
  | {kind:"performance_payment";subjectId:string;reviewerUserId:string;decision:Decision;note:string;idempotencyKey:string};
...
export function executeCommercialApproval(pool:Pool,command:CommercialApprovalCommand){
  ...
  return decidePerformancePayment(pool,{statementId:command.subjectId,reviewerUserId:command.reviewerUserId,decision:command.decision,note:command.note,idempotencyKey:command.idempotencyKey});
}
