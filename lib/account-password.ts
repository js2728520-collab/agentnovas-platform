...
export async function changeAccountPassword(pool: Pool, input: {
  ...
  try {
    ...
    const user = input.sessionTokenHash && input.currentPasswordHash
      ? { password_hash: input.currentPasswordHash }
      : (await client.query<{ password_hash: string }>("SELECT password_hash FROM users WHERE id = $1 FOR UPDATE", [input.userId])).rows[0];
    ...
    if (input.sessionTokenHash) {
      const changed = await client.query<{ changed: boolean }>(
        "SELECT client_change_password($1,$2,$3,$4) AS changed",
        [input.sessionTokenHash,user.password_hash,passwordHash,now],
      );
      ...
      await client.query(`
        INSERT INTO audit_logs(id,actor_user_id,action,subject_type,subject_id,after_json,ip_address,user_agent,created_at)
