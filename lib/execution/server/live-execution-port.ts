 ...
 *
 * **本文件不打开实盘路由。** 是否真的往交易所发单由 `execution_live_routing`
 * 授权表按 (交易所, 环境) 逐条控制，默认全关。
 *
 * 注意：早期版本用过一个 `LIVE_EXECUTION_ENABLED` 环境变量，**它已经不存在了**，
 * 代码里零处读取。照着旧文档配那个变量既打不开也关不掉任何东西——真正的开关在
 * 运营端「风控 → 实盘路由」，走 maker/checker。
 *
 * 更根本的是：实盘链路目前还没接通，见
 * packages/domain/src/execution/live-readiness.ts。
 *
