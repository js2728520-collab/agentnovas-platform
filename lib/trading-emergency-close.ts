...
export async function closeOkxDemoTrade(position: OpenTrade, account: typeof exchangeAccounts.$inferSelect, now: string): Promise<EmergencyCloseResult> {
  if (position.side !== "buy") return { tradeId: position.id, symbol: position.symbol, status: "unsupported", message: "当前仅支持现货多仓的一键平仓" };
  try {
    ...
    if (position.status === "closing" && position.closeExchangeOrderId) {
      ...
      if (order.state === "filled" && order.filledQuantity > 0 && order.averagePrice > 0) {
        ...
        await getDb().batch(statements as never);
    ...
    const order = await placeOkxDemoMarketOrder({ credentials, symbol: position.symbol, side: "sell", quantity: position.quantity, clientOrderId: `EST${Date.now().toString(36)}${crypto.randomUUID().slice(0, 8)}` });
