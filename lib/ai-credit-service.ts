...
export async function mutateAiCredits(client: PoolClient, input: {
  ...
  const prior = await client.query<{ id:string;balance_available:string;balance_reserved:string;user_id:string;entry_type:string;source_type:string;source_id:string;available_delta:string;reserved_delta:string }>(
    `SELECT e.id,e.balance_available::text,e.balance_reserved::text,a.user_id,e.entry_type,e.source_type,e.source_id,e.available_delta::text,e.reserved_delta::text FROM ai_credit_ledger_entries e JOIN ai_credit_accounts a ON a.id=e.account_id WHERE e.idempotency_key = $1`,
    [input.idempotencyKey],
  );
  if (prior.rows[0]) {const row=prior.rows[0];if(row.user_id!==input.userId||row.entry_type!==input.type||row.source_type!==input.sourceType||row.source_id!==input.sourceId||BigInt(row.available_delta)!==input.availableDelta||BigInt(row.reserved_delta)!==input.reservedDelta)throw new ResearchApiError("IDEMPOTENCY_KEY_COLLISION","Credits 幂等键已绑定其他变更",409);return { entryId:row.id,available:row.balance_available,reserved:row.balance_reserved,created:false };}
...
export async function settleAiCreditReservation(pool:Pool,input:{reservationId:string;idempotencyKey:string;requestId:string;actorUserId?:string;costModelVersion:"token-cost-v1";trustedUsage:{source:"provider_metering";usageId:string;inputTokens:number;outputTokens:number}}){
  ...
  const cost=calculateTokenCost({modelVersion:input.costModelVersion,usageReliable:true,rateReliable:true,inputTokens:input.trustedUsage.inputTokens,outputTokens:input.trustedUsage.outputTokens});const actualCredits=BigInt(cost.credits);const client=await pool.connect();try{await client.query("BEGIN");
    const reservation=await client.query<{estimated_credits:string;status:string;user_id:string}>(`SELECT r.estimated_credits::text,r.status,a.user_id FROM ai_credit_reservations r JOIN ai_credit_accounts a ON a.id=r.account_id WHERE r.id=$1 FOR UPDATE`,[input.reservationId]);const row=reservation.rows[0];if(!row)throw new ResearchApiError("AI_CREDIT_RESERVATION_NOT_FOUND","Credits 预留不存在",404);if(row.status==="settled"){const prior=await client.query<{idempotency_key:string;created_by_user_id:string|null;usage_json:{usageId?:string}}>(`SELECT idempotency_key,created_by_user_id,usage_json FROM ai_credit_ledger_entries WHERE reservation_id=$1 AND entry_type='settle'`,[input.reservationId]);if(prior.rows[0]?.idempotency_key!==input.idempotencyKey||prior.rows[0]?.created_by_user_id!==(input.actorUserId??null)||prior.rows[0]?.usage_json?.usageId!==input.trustedUsage.usageId)throw new ResearchApiError("IDEMPOTENCY_KEY_COLLISION","Credits settle 重放上下文不一致",409);await client.query("COMMIT");return {reservationId:input.reservationId,created:false};}if(row.status!=="reserved")throw new ResearchApiError("AI_CREDIT_RESERVATION_STATE_CONFLICT","Credits 预留状态冲突",409);const estimated=BigInt(row.estimated_credits);if(actualCredits>estimated)throw new ResearchApiError("AI_CREDIT_RESERVATION_EXCEEDED","可信用量成本超过预留，拒绝自动补扣",409);
