...
export async function mutateAiCredits(client: PoolClient, input: {
  ...
  const prior = await client.query<{ id:string;balance_available:string;balance_reserved:string;user_id:string;entry_type:string;source_type:string;source_id:string;available_delta:string;reserved_delta:string;reservation_id:string|null;cost_model_version:string|null;usage_json:unknown;created_by_user_id:string|null }>(
  ...
  if (prior.rows[0]) {const row=prior.rows[0];if(row.user_id!==input.userId||row.entry_type!==input.type||row.source_type!==input.sourceType||row.source_id!==input.sourceId||BigInt(row.available_delta)!==input.availableDelta||BigInt(row.reserved_delta)!==input.reservedDelta||row.reservation_id!==(input.reservationId??null)||row.cost_model_version!==(input.costModelVersion??null)||row.created_by_user_id!==(input.actorUserId??null)||canonicalPayloadHash(row.usage_json) !== canonicalPayloadHash(input.usage??null))throw new ResearchApiError("IDEMPOTENCY_KEY_COLLISION","Credits 幂等键已绑定其他变更",409);return { entryId:row.id,available:row.balance_available,reserved:row.balance_reserved,created:false };}
...
export async function reserveAiCredits(client:PoolClient,input:{userId:string;credits:bigint;sourceType:string;sourceId:string;idempotencyKey:string;requestId:string;expiresAt:string;actorUserId?:string}){
  ...
  const existing=await client.query<{id:string;user_id:string;estimated_credits:string;expires_at:Date;source_type:string|null;source_id:string|null;created_by_user_id:string|null}>(`SELECT r.id,a.user_id,r.estimated_credits::text,r.expires_at,e.source_type,e.source_id,e.created_by_user_id FROM ai_credit_reservations r JOIN ai_credit_accounts a ON a.id=r.account_id LEFT JOIN ai_credit_ledger_entries e ON e.reservation_id=r.id AND e.entry_type='reserve' WHERE r.idempotency_key=$1 FOR UPDATE OF r`,[input.idempotencyKey]);
  if(existing.rows[0]){const row=existing.rows[0];if(row.user_id!==input.userId||BigInt(row.estimated_credits)!==input.credits||row.expires_at.toISOString()!==new Date(input.expiresAt).toISOString()||row.source_type!==input.sourceType||row.source_id!==input.sourceId||row.created_by_user_id!==(input.actorUserId??null))throw new ResearchApiError("IDEMPOTENCY_KEY_COLLISION","Credits 预留幂等键已绑定其他变更",409);return {reservationId:row.id,created:false};}
...
export async function settleAiCreditReservation(pool:Pool,input:{reservationId:string;idempotencyKey:string;requestId:string;actorUserId?:string;costModelVersion:"token-cost-v1";trustedUsage:{source:"provider_metering";usageId:string;inputTokens:number;outputTokens:number}}){
  if(input.trustedUsage.source!=="provider_metering"||!input.trustedUsage.usageId.trim())throw new ResearchApiError("AI_USAGE_NOT_RELIABLE","仅接受供应商计量用量",422);
  const cost=calculateTokenCost({modelVersion:input.costModelVersion,usageReliable:true,rateReliable:true,inputTokens:input.trustedUsage.inputTokens,outputTokens:input.trustedUsage.outputTokens});const actualCredits=BigInt(cost.credits);const client=await pool.connect();try{await client.query("BEGIN");
    const reservation=await client.query<{estimated_credits:string;status:string;user_id:string}>(`SELECT r.estimated_credits::text,r.status,a.user_id FROM ai_credit_reservations r JOIN ai_credit_accounts a ON a.id=r.account_id WHERE r.id=$1 FOR UPDATE`,[input.reservationId]);const row=reservation.rows[0];if(!row)throw new ResearchApiError("AI_CREDIT_RESERVATION_NOT_FOUND","Credits 预留不存在",404);if(row.status==="settled"){const prior=await client.query<{idempotency_key:string;created_by_user_id:string|null;cost_model_version:string|null;usage_json:unknown}>(`SELECT idempotency_key,created_by_user_id,cost_model_version,usage_json FROM ai_credit_ledger_entries WHERE reservation_id=$1 AND entry_type='settle'`,[input.reservationId]);const expectedUsage={...input.trustedUsage,costModelVersion:cost.modelVersion,credits:cost.credits};if(prior.rows[0]?.idempotency_key!==input.idempotencyKey||prior.rows[0]?.created_by_user_id!==(input.actorUserId??null)||prior.rows[0]?.cost_model_version!==input.costModelVersion||canonicalPayloadHash(prior.rows[0]?.usage_json)!==canonicalPayloadHash(expectedUsage))throw new ResearchApiError("IDEMPOTENCY_KEY_COLLISION","Credits settle 重放上下文不一致",409);await client.query("COMMIT");return {reservationId:input.reservationId,created:false};}if(row.status!=="reserved")throw new ResearchApiError("AI_CREDIT_RESERVATION_STATE_CONFLICT","Credits 预留状态冲突",409);const estimated=BigInt(row.estimated_credits);if(actualCredits>estimated)throw new ResearchApiError("AI_CREDIT_RESERVATION_EXCEEDED","可信用量成本超过预留，拒绝自动补扣",409);
    await mutateAiCredits(client,{userId:row.user_id,type:"settle",availableDelta:estimated-actualCredits,reservedDelta:-estimated,sourceType:"ai_credit_reservation",sourceId:input.reservationId,idempotencyKey:input.idempotencyKey,requestId:input.requestId,actorUserId:input.actorUserId,reservationId:input.reservationId,costModelVersion:input.costModelVersion,usage:{...input.trustedUsage,costModelVersion:cost.modelVersion,credits:cost.credits}});await client.query(`UPDATE ai_credit_reservations SET status='settled',settled_credits=$2,version=version+1,updated_at=now() WHERE id=$1`,[input.reservationId,actualCredits.toString()]);await client.query("COMMIT");return {reservationId:input.reservationId,created:true,settledCredits:cost.credits};
