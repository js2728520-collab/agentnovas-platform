...
| --- | --- | --- | --- |
| Client | 受邀客户 | 商业披露确认、试用/会员、credits、三策略 paper、七阶段记录、优盾 USDT 充值、平台 Demo 证据、通知 | 内部运营/运维数据；提现/划转；上传交易所密钥；真实下单 |
...
`/wallet` 保留只读余额和不可变账本。`/wallet/deposits` 开放优盾 USDT deposit-only 订单：客户端选择网络和预计金额，地址只能来自优盾签名接口；验签回调进入 `MANUAL_REVIEW`，由不同 Operations maker/checker 批准后才原子写入钱包和账本。未配置商户号、API Key、专属节点、回调地址或币种编号时返回明确 503，不生成地址、二维码或成功回执。提现、划转、自动扣款和退款仍不属于本版本。
...
- 不做社区策略市场、作者分润、提现/划转、自动扣款、自动退款或无人复核入账；优盾链上地址和回调是唯一例外且受 ADR-0015 约束。
