...
| --- | --- | --- | --- |
| Client | 受邀客户 | 法务同意、试用/会员、credits、三策略 paper、七阶段记录、平台 Demo 证据、通知 | 内部运营/运维数据；客户充值；上传交易所密钥；真实下单 |
...
2. Client 完成邮箱验证、当前版本服务条款/隐私/风险披露同意。
3. 系统开通 3 天 trial；到期规则由法务正文约束。
...
10. 到期停止新开仓并进入只读/平仓处理，具体退出和退款口径以定稿法务文件为准。
...
如果法务在 D3 前不批准“模拟收益分成”及退款规则，付费上线 Gate 失败，工程不得自行改收费口径。
...
Beta 只有在 `docs/quality/ACCEPTANCE_AND_RELEASE_GATES.md` 全部通过后才可开放。法务主体、服务地区、隐私、条款、风险披露、模拟收益分成合法性意见和退款规则任一缺失，或任一跨 audience、重复计费、假状态、真实订单可达、迁移/恢复/E2E 失败，均为否决项。
