...
| G2 market data and multi-market gate | `PARTIAL` | `docs/quality/FULL_PLATFORM_V3_GATES.md`, `tests/market-route-contract.test.mjs`, `tests/maintenance-integration-catalog.test.mjs`, `lib/market-quotes.ts`, `lib/market-candles.ts`, `lib/maintenance-integration-catalog.ts` | Quote/candle route architecture is locally contract-tested and delegates to reusable display/research services; Maintenance now exposes server-side configuration entry points and missing environment-variable names without secrets. Real provider readiness, authorization, WebSocket, stateful failover, and target-environment evidence remain unverified. |
...
| G6 Operations and Maintenance gate | `PARTIAL` | `docs/quality/FULL_PLATFORM_V3_GATES.md`, `docs/README.md`, `docs/quality/QUALITY_RELEASE_EVIDENCE.md`, `apps/maintenance/ui/source-integrations-workspace.tsx`, `lib/maintenance-integration-catalog.ts` | Maintenance source integration status now separates configuration entry, configured, enabled, health, stale, and test evidence without returning secret values. The wider G6 UI/RBAC/observability surface and target-environment evidence were not re-audited here. The recorded development-toolchain exception (17 high/moderate vulnerabilities, including 9 high) remains a named release blocker pending remediation/re-scan before paid Beta invitations. |
