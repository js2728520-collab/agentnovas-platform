...
| G2 market data and multi-market gate | `PARTIAL` | `docs/quality/FULL_PLATFORM_V3_GATES.md`, `tests/market-route-contract.test.mjs`, `lib/market-quotes.ts`, `lib/market-candles.ts` | Quote/candle route architecture is now locally contract-tested and delegates to reusable display/research services; real provider readiness, WebSocket, stateful failover, and target-environment evidence remain unverified. |
