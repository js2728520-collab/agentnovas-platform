...
> 文档状态：`FOUNDATION`。研究与回测闭环继续作为 V3 策略准入基础；具体市场、门槛、作者审核和跟单合同由 V3 PRD/Spec 版本化配置。

状态：已批准实施

批准依据：用户于 2026-08-18 要求建设专业 AI 对话，以及策略生成后的保存、查看和回测能力；该历史基础不构成 QuantDinger 仓库、代码或演示的移植授权。

实现分支：`codex/ai-assistant-strategy-dsl`

## 1. 假设与边界

1. 研究与回测能力按 AgentNovas 自有对话、受限 DSL 和确定性验证合同原生演进，不以第三方代码、前端或演示作为移植基准。
