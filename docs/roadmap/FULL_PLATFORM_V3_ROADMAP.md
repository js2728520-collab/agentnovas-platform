...
3. 结构化策略、回测和准入合同。
4. 客户投稿/审核/上架/下架。
5. 策略详情、作者、费用和风险披露；只验证不可变合同快照，不在 G3 内证明真实结算。
6. Paper/Demo 跟单配置、版本快照、暂停/停止。

退出条件：G3 通过；S0 仍只允许具备独立 G3 证据的 Paper/Demo 策略市场与 Paper 跟单配置。真实订单跟单等待 G4/G4A，作者/平台真实分账、收益分成结算和 live 资金副作用等待独立商业/账本 Gate。
