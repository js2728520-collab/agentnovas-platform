...
| `/api/maintenance/configuration-versions/[id]/tests` | POST | M | CURRENT（T3.1c-FF1）；注册配置族只接收原因并由服务端生成确定性结果/证据；尚未注册的其他配置族保留人工 passed/failed 证据，审批后均禁止补写 |
...
| `/api/maintenance/configuration-versions/[id]/activation` | POST | M | CURRENT（T3.1c-FF1）；到期激活或回滚到同流曾生效的已验证版本；`client.strategy_research` 从下一次请求接管全局模块开关，其他配置族不执行具体副作用 |
