...
- `specs/MAINTENANCE_AI_USAGE_ANALYTICS_SPEC.md`：T3.9a Maintenance AI 用量安全聚合合同；按 UTC 请求创建 cohort 展示可信成功 Token、settled Credits、已记录非取消失败率和脱敏多维分析；这些是计量/结算事实，不表示可切换的 `provider_usage` 模式已启用；后者与固定价格 consumer 统一属于 S0 之外的 T3.9b。
...
- 客户充值仅允许优盾 deposit-only 通道；未验签或未双人复核不得入账。**平台服务余额提现**已由
  ADR-0024 纳入产品范围（P-09），但 G5 全部通过前接口固定拒绝；划转、自动扣款、真实退款仍不可达。
  客户交易所账户的提现权限永久禁止（INV-11，迁移 0045 数据库约束），与平台余额提现是两件事。
