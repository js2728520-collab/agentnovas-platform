...
**未做：** 第 3 步（自动风控在每个周期上调用）判定函数已在路径里被调用，但**风控触发后没有
写回订阅状态**——`evaluateFollowRisk` 的结论目前只用于挡新开仓，没有把订阅置成
`risk_blocked`，也没有写 `strategy_follow_risk_events`。跟单参数的 configuring →
user_confirmed 确认流程与界面、运营端风控工作台界面、作者分账的账本分录同样未做。
