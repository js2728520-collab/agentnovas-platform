...
## 104. 2026-08-25 `provider_usage` 模式归入 T3.9b

需求方确认：可配置的 `provider_usage` runtime consumer 与模式切换不属于当前 S0，也不作为后续 S0 增量；它与固定 Credits consumer、模型/功能价格分档统一归入 T3.9b。P-08 中存在 `provider_usage` 产品模式只表示参数已获确认，不表示运行时 consumer、切换配置或发布 Gate 已存在。

现有 provider 返回的可信 usage、reservation/settle/release 和不可变 Credits 流水继续作为当前计量与结算事实。这些事实与“可以在固定价格和 `provider_usage` 间切换”是两个不同层次：前者已有当前证据，后者必须在 T3.9b 独立实现严格 schema、确定性 tester、历史 pin、最小权限 consumer、回滚和 Gate。文档不得用前者证明后者已配置、启用或属于 S0。

本次只同步仍有歧义的当前入口、目标规格和状态矩阵；`V3_RELEASE_SCOPE.md`、任务计划/看板与版本化配置规格已经在前一切片准确表达该边界，未为制造差异重复改写。历史 handoff 条目保留原时间点措辞。未修改业务代码、迁移或生产配置；未执行生产迁移、未接触生产数据库、未推送、未部署。
...
## 105. 2026-08-25 Prompt 与 Skill S0/T3.10 边界

需求方确认：Prompt 版本治理保留为目标设计，现有 Prompt family、确定性 tester、PS2 consumer/任务固定和 PS3 工作台作为当前可复用资产记录；它们仍须按独立证据与 Gate 判定，不能由配置版本 `ACTIVE` 推导为发布通过。

Skill v1 的 PS-01–PS-06 已于 2026-08-24 全部冻结，声明式 schema/tester 和工作台资产也已存在，但 runtime consumer 尚未实现，没有 Agent 加载 active Skill。可编辑 Skill runtime execution 不属于当前 S0，统一留给 T3.10 独立完成最小权限任务加载、版本固定、失败关闭、回滚和 Gate。Prompt PS2、通用配置 Worker 或 active/current 状态均不能替代 T3.10。

本次同步确认单、PRD、文档入口/状态矩阵、版本化配置规格、Maintenance 目标规格、能力迁移矩阵、roadmap 与 S0 范围；历史 handoff 条目保留原时间点。未修改业务代码、迁移或生产配置；未执行生产迁移、未接触生产数据库、未推送、未部署。

## 106. 2026-08-25 策略市场与跟单的 S0/G3 边界

需求方确认：S0 可以包含已通过独立 G3 证据的 Paper/Demo 策略市场与 Paper 跟单配置。真实订单跟单、作者/平台真实分账、收益分成结算以及任何 live 资金副作用均排除在 S0；真实跟单等待 G4/G4A，真实商业结算等待独立商业/账本 Gate。Paper、Demo、Live 的回执、账本、收益和状态不得混写，目标规格也不得被解释为当前授权。

本次只同步当前/目标策略市场、系统和 Client 规格、S0 范围、G3 Gate、roadmap、PRD、文档状态矩阵与能力迁移矩阵；保留历史 handoff、质量证据和其它 Gate 记录的原时间点事实。未修改业务代码、数据库迁移或生产配置；未执行生产迁移、未接触生产数据库、未推送、未部署。

验证：`git diff --check`、策略市场边界检索和文档-only 文件范围检查通过后提交本切片。G3/G4/G4A、商业/账本和目标环境证据仍未因本文同步而通过。

## 107. 2026-08-25 S0 Paper-only 模拟费用边界

需求方在策略市场收费范围冲突中选择“模拟计算与披露”：S0 可以保存不可变费用合同快照，并计算、展示 Paper-only 的模拟收益分成和作者/平台模拟分配。所有相关页面、结果和导出必须明确标注“模拟、不可提现、未实际结算”，不能被解释为客户真实收费、作者真实收入或平台已结算收入。

S0 不得因此扣客户服务余额、生成真实应收或发票、增加作者收益余额、触发支付或退款，也不得向资金账本写入分录。真实订单跟单继续等待 G4/G4A；真实订阅费、收益分成、作者/平台分账及其它真实商业副作用继续等待独立商业/账本 Gate。

本次只同步 PRD、跨团队功能说明、System/Client 目标规格、S0 范围、G3 Gate、roadmap、版本化配置边界、文档状态矩阵和能力迁移矩阵；未修改业务代码、数据库迁移或生产配置。验证仅覆盖文档一致性和 diff 完整性，不表示 G3、商业/账本 Gate 或发布 Gate 已通过；未执行生产迁移、未接触生产数据库、未推送、未部署。

## 108. 2026-08-25 S0 保留既有会员/Credits 事实

需求方选择选项 A：S0 保留已有且具备独立证据的会员、Credits reservation/settle/release、可信 provider usage、不可变 Credits ledger 等现有事实；这不等于固定 Credits consumer、模型/功能价格分档或 `provider_usage` 可切换 runtime 模式已经启用。后述能力继续统一归入 S0 之外的 T3.9b，并须独立完成 consumer、历史 pin、回滚和 Gate。

Paper-only 模拟收益分成及作者/平台模拟分配必须与上述 Credits 和真实资金事实隔离：不得扣 Credits 或客户服务余额，不得进入 Credits ledger，不得生成真实应收、发票或作者收益余额，不得触发支付/退款或写入资金账本。已有 Credits 事实也不得被用作模拟费用或真实结算已通过的证据。

本次仅同步 S0 范围、PRD、Current/Target 规格、roadmap、文档状态矩阵和版本化配置边界；未修改业务代码、数据库迁移或生产配置。验证仅覆盖文档一致性和 diff 完整性；未执行生产迁移、未接触生产数据库、未推送、未部署。

## 109. 2026-08-25 S0 支付、退款与优惠效果边界

需求方选择选项 A：保留支付、人工退款、优惠码/折扣码/优惠券的目标设计和服务端安全配置入口，但真实 payment/refund/coupon runtime consumer、外部结果和资金副作用不属于 S0。未通过独立商业/账本 Gate 时，Client、Operations 和 Maintenance 必须展示 `not_configured`、`disabled` 或 `unverified` 等真实状态；配置版本 `ACTIVE`、人工审批、provider 连通或健康均不得被解释为支付成功、退款完成或优惠已应用。

S0 不得因这些目标合同或配置入口扣客户服务余额或 Credits，不得生成真实应收、发票或作者收益余额，不得写入资金账本。支付、退款和优惠的独立 schema、确定性 tester、最小权限 consumer、不可变历史 pin、回滚、provider 外部证据和商业/账本 Gate 完成前，所有外部写入继续失败关闭。

本次仅同步目标文档、S0 范围、Gate、roadmap、版本化配置边界、能力迁移矩阵和本交接记录；未修改业务代码、数据库迁移或生产配置。验证仅覆盖文档一致性和 diff 完整性，不表示支付、退款、优惠、商业/账本或发布 Gate 已通过；未执行生产迁移、未接触生产数据库、未推送、未部署。

## 110. 2026-08-25 P-01 八家交易所范围对齐

需求方再次确认 P-01 一共八家交易所，顺序沿用唯一真源 `packages/contracts/src/product-parameters.ts`：Binance、OKX、Coinbase、Crypto.com、Kraken、Gate.io、Bitget、HTX；MetaMask 首期不接入。

本切片修正 PRD、跨团队目标功能说明、能力迁移矩阵和行情合同中仍把目标范围写成“五家 + MetaMask”或把 P-01 顺序写成未确认的现行表述。PRD 的“冻结前原始清单”和 2026-08-23 readiness 评估保留原时间点事实，不作为现行目标；八家目标范围也不表示任何 provider 已完成认证、通过 G4/G4A、进入 S0 或获准真实交易。

本次仅修改文档，未修改产品参数代码、业务代码、数据库迁移或生产配置。验证覆盖 P-01 现行文档检索、产品参数测试和 `git diff --check`；未执行生产迁移、未接触生产数据库、未推送、未部署。

## 111. 2026-08-25 P-03 六个股票市场范围对齐

需求方确认 P-03 冻结为六个股票市场：美国、A 股、港股、韩股、日股、澳股。首期股票行情按 P-03 以 15 分钟延迟提供，运维端可控可见性；实时行情只能在后续取得授权并通过独立升级 Gate 后开放。

本切片修正 PRD、跨团队目标功能说明、Client 目标规格、行情合同、路线图、能力迁移矩阵、文档状态矩阵和 Phase 2 任务计划中仍把目标市场写成四个、按“优先/随后”逐步冻结，或把首期股票数据概括为实时的现行表述。当前 API 的四个兼容市场与 40 个静态标的仍是当前实现事实，不被改写成六个市场已可用；真实 provider、授权、再分发、SLA、WebSocket、实时升级和 G2 仍需独立证据与 Gate。PRD 的“冻结前原始清单”和历史交接/评估记录保留其时间点事实，不作为当前目标。

本次仅修改文档，未修改产品参数代码、业务代码、数据库迁移、provider、生产配置或 UI。验证覆盖 P-03 现行文档检索、产品参数与 provider registry 定向测试、`git diff --check` 和文档-only diff 审查；未执行生产迁移、未接触生产数据库、未推送、未部署。

## 112. 2026-08-25 P-04 不做 QuantDinger 移植参考

需求方选择选项 A：P-04 不以 QuantDinger 代码仓库、演示地址或可移植版本作为 AI 助手的移植基准。AI 助手、研究与回测能力按 AgentNovas 自有七阶段决策链、受限策略 DSL、确定性校验、回测、准入和审计合同原生演进；历史文档中保留的第三方提及仅表示历史产品/架构参考，不构成当前实施指令或移植授权。

本切片修正 PRD、跨团队目标功能说明、能力迁移矩阵、AI/Prompt 基础说明、G3 Gate、roadmap 和 Phase 4 任务计划中仍把 QuantDinger 写成活动移植基准、差异清单依赖或待冻结前置条件的表述。当前 AI 对话、策略 DSL、回测和确定性验证资产继续按 AgentNovas 自有方向核验；统一入口、信息架构、策略准入、Paper/Demo 策略市场和其它 G3 条目仍需独立实施与验收。历史 ADR、日期化评估快照及 PRD 的“冻结前原始清单”保留原时间点事实，不作为当前目标或执行指令。

本次仅修改文档，未修改 AI/策略业务代码、产品参数代码、数据库迁移、provider、生产配置或 UI。验证覆盖 P-04 现行文档检索、相关策略/Prompt 合同定向测试、`git diff --check` 和文档-only diff 审查；未执行生产迁移、未接触生产数据库、未推送、未部署。

## 113. 2026-08-26 P-05 策略准入边界对齐

需求方选择选项 A：策略上架要求至少 180 天历史回测、至少 30 笔成交、正收益、按风险等级限制最大回撤 10%/15%/20%，并必须经过人工审核；不设置强制模拟盘运行时长。运营端可调整可配置阈值，但不得放宽已冻结的 P-05 边界；任一门槛未满足必须明确标记 `NOT_QUALIFIED`，不得以口头结论、静态评分或展示文案替代确定性判定。

本切片修正 PRD、跨团队目标功能说明、能力迁移矩阵、G3 Gate 和 Phase 4 任务计划中仍把“规定模拟盘时长”写成当前上架条件，或没有完整列出 P-05 的回测、成交、收益、回撤、人工审核与 `NOT_QUALIFIED` 语义。ADR-0024、产品参数源和既有策略准入实现中已经记录/实现的正确边界保留；G3、浏览器和完整证据仍未因本次文档修改而通过。

本次仅修改文档，未修改策略准入代码、产品参数代码、数据库迁移、provider、生产配置或 UI。验证覆盖 P-05 现行文档检索、相关策略准入合同定向测试、`git diff --check` 和文档-only diff 审查；未执行生产迁移、未接触生产数据库、未推送、未部署。

## 114. 2026-08-26 P-06 策略收费与分账边界对齐

需求方选择选项 A：策略跟单不收固定订阅费，只对符合合同且超过客户-策略高水位线的实盘周收益收取 20% 绩效分成；按 UTC 自然周计算，亏损周不收费，先补回历史高水位线后才重新计费。作者与平台按 50%/50% 分账，已完成结算的费用不退款，历史跟单和结算事实固定不可变费用快照。
...
G3 focused 浏览器旅程以端口偏移 `1000` 运行，避开本机非项目进程占用的 `localhost:3002`；真实 Chromium 1/1 通过，覆盖策略广场与我的跟单、披露复选框禁用/启用、筛选/排序、历史表现不公开 DSL、Paper-only 文案、零 POST `/orders` 或 `/deployments`、四断点、键盘与 axe。测试中发现新增排序选项使 `getByText("跟随人数")` 命中下拉选项和详情指标两个元素，已改为精确文本定位；这不是放宽断言，而是恢复原本要验证的详情指标语义。
