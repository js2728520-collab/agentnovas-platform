...
- 解析结果绑定具体策略版本、canonical market/instrument、provider symbol 和 capability 版本。
...
- 解析后的绑定带策略版本 ID 与 fingerprint；同一输入稳定产生同一 fingerprint，任一语义字段变化都会改变 fingerprint。
...
调用方必须从已认证 Session、账户仓储和受控 provider registry 构造快照。浏览器提交同形对象不构成可信事实。
...
fingerprint 使用固定 domain tag、fingerprint version 和严格规范化后的绑定字段计算 SHA-256。它不包含时间、健康、延迟、sequence、secret 或 provider endpoint。调用方不得对未规范化浏览器 JSON 直接哈希。
...
7. 任何 blocked 结果都必须保持 `binding=null`、`fingerprint=null`，不得隐式改用公共源或平台默认源。
8. 解析成功只证明选择在该 capability 快照下可绑定。Runtime 仍须执行 T2.2/T2.3 的 sequence、freshness、price integrity、当前健康和 stale Gate。
9. `authorizesOrders` 固定为 false；本合同不能替代账户交易权限、风控、named live Gate、Execution Service 或资金 Gate。
10. 退出、撤单和事故处置不得简单复用“禁止新开仓”的结果；由 Runtime/Execution 合同独立处理。
...
1. 策略版本或客户选择版本保存选源意图；不得写进 `specification_json` DSL。
2. deployment binding version 保存账户解析后的实际 provider/source policy 与 fingerprint；账户或 provider 配置变化不得静默改写既有 deployment。
3. Runtime cycle/market snapshot 记录 binding/policy fingerprint、最终 selected provider、quality 和数据集证据。

同 DSL 更换行情源后，旧回测、评分和 `STANDARD_VERIFIED` 不得自动沿用。ADR-0018 当前共享决策轮身份缺少 source binding；Runtime 接入前必须把 binding/policy fingerprint 纳入轮身份，避免不同来源错误共享同一决策和快照。
...
- 任一语义字段变化都会改变 fingerprint，纯字段顺序变化不影响结果。
