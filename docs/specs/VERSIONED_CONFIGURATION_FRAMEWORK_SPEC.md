...
- **S0 模拟费用边界：** Paper-only 模拟收益分成和作者/平台模拟分配可以引用不可变费用合同快照，但必须标注“模拟、不可提现、未实际结算”；该计算不得扣服务余额、生成真实应收/发票/作者收益余额、触发支付/退款或写入资金账本。配置版本 `active` 不能把模拟事实提升为真实结算。
...
- **S0 支付/退款/优惠边界：** 可以保留目标 schema、工作流和服务端安全配置入口，但 S0 不启用真实 consumer 或外部写入。未通过独立商业/账本 Gate 时只能投影 `not_configured`、`disabled` 或 `unverified`，不得由 `active`、人工审批、连通测试或健康状态推导支付成功、退款完成或优惠已应用；不得产生服务余额/Credits 扣减、真实应收/发票/作者余额或资金账本分录。
