...
实施快照（2026-08-24）：T3.1a 通用发布内核和 Maintenance-only API、T3.1b-UI
`/configurations` 工作台已完成，包含不可变版本、顶层差异、测试证据、禁止自审、明确
offset/UTC 预览、调度、current、激活和已生效历史回滚。到期激活器以及品牌/域名/协议等
具体消费者尚未实现，页面明确不声称这些配置已接管运行时。
