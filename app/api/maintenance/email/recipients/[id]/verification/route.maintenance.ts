...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      ...
      throw new ResearchApiError(code, "验证码动作或验证码无效", 422);
    }
      ...
      try {
        const response = command.action === "verify"
          ? await verifyEmailTestRecipient(client, {
            actorUserId: user.id,recipientId,code: command.code,request,
          })
          : await resendEmailTestRecipientVerification(client, {
            actorUserId: user.id,recipientId,request,idempotencyHash,
          });
