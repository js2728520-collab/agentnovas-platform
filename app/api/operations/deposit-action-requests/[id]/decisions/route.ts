...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const { user } = await requireAccessPermission(request, "ops.deposits.action_approve");
    ...
    const action = await pool.query<{ requested_by_user_id: string; status: string }>("SELECT requested_by_user_id, status FROM deposit_action_requests WHERE id = $1 LIMIT 1", [id]);
