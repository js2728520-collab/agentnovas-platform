...
export async function GET(request: Request) {
  ...
  try {
    if (category === "crypto") { const { data: row, base } = await fetchPublicMarketJson<{ lastPrice: string; priceChange: string; priceChangePercent: string; highPrice: string; lowPrice: string; volume: string; quoteVolume: string; openPrice: string }>(`/api/v3/ticker/24hr?symbol=${cryptoProvider(symbol)}`, 5000); return Response.json({ symbol, price: Number(row.lastPrice), change: Number(row.priceChange), changePercent: Number(row.priceChangePercent), high: Number(row.highPrice), low: Number(row.lowPrice), volume: Number(row.quoteVolume || row.volume), open: Number(row.openPrice), live: true, source: publicMarketProviderName(base), updatedAt: new Date().toISOString() }, { headers: { "cache-control": "no-store" } }); }
