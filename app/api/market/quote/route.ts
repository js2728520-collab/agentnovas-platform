...
async function yahooQuote(symbol: string, timeoutMs: number) {
  const response = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=1d&interval=5m`, { headers: { accept: "application/json" }, cache: "no-store", signal: AbortSignal.timeout(timeoutMs) });
...
export async function GET(request: Request) {
  ...
  try {
    if (category === "crypto") {
      return Response.json({ symbol, ...(await getPublicMarketQuote(selection.source, symbol, selection.requestTimeoutMs)), exchangeName: selection.source.displayName, selectionMode: selection.mode }, { headers: { "cache-control": "no-store" } });
    }
    const provider = yahooSymbols[symbol] || symbol;
    return Response.json({ symbol, ...(await yahooQuote(provider, selection.requestTimeoutMs)), exchange: selection.source.key, exchangeName: selection.source.displayName, selectionMode: selection.mode, updatedAt: new Date().toISOString() }, { headers: { "cache-control": "no-store" } });
  } catch (error) { return Response.json({ symbol, price: 0, change: 0, changePercent: 0, high: 0, low: 0, volume: 0, open: 0, live: false, source: "unavailable", exchange: selection.source.key, exchangeName: selection.source.displayName, selectionMode: selection.mode, updatedAt: new Date().toISOString(), error: error instanceof Error ? error.message : "行情源暂时不可用" }, { status: 503, headers: { "cache-control": "no-store" } }); }
}
