...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = await request.json() as { name?: string; summary?: string; symbols?: string[]; riskLevel?: "low" | "medium" | "high"; publicationMode?: "marketplace" | "self_use"; conversation?: unknown[]; specification?: Record<string, unknown> };
    if (!body.name?.trim() || !body.summary?.trim()) return Response.json({ error: "策略名称和说明为必填" }, { status: 400 });
    const now = new Date().toISOString();
    const changes = { name: body.name.trim(), summary: body.summary.trim(), symbolsJson: JSON.stringify(body.symbols || []), riskLevel: body.riskLevel || "medium" as const, publicationMode: body.publicationMode === "self_use" ? "self_use" as const : "marketplace" as const, conversationJson: JSON.stringify(body.conversation || []), specificationJson: JSON.stringify(body.specification || {}), version: current.version + 1, status: "draft" as const, rejectionReason: null, updatedAt: now };
    await db.batch([
