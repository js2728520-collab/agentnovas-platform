import { and, eq, isNull } from "drizzle-orm";
import { getDb } from "@/db";
import { auditLogs, authTokens, sessions, users } from "@/db/schema";
import { canDeactivateMember } from "@/lib/permissions";
import { requireUser, responseError } from "@/lib/session";

const roles = ["hq_admin", "branch_admin", "manager", "supervisor"] as const;

export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const actor = await requireUser(request, [...roles]);
    const { id } = await params;
    const db = getDb();
    const member = (await db.select({
      id: users.id,
      email: users.email,
      role: users.role,
      organizationId: users.organizationId,
      reportsToUserId: users.reportsToUserId,
      status: users.status,
    }).from(users).where(eq(users.id, id)).limit(1))[0];

    if (!member) return Response.json({ error: "成员账户不存在" }, { status: 404 });
    if (!canDeactivateMember(actor, member)) return Response.json({ error: "无权停用该成员账户" }, { status: 403 });

    const now = new Date().toISOString();
    await db.batch([
      db.update(users).set({ status: "frozen", updatedAt: now }).where(and(eq(users.id, member.id), eq(users.status, "active"))),
      db.update(sessions).set({ revokedAt: now }).where(and(eq(sessions.userId, member.id), isNull(sessions.revokedAt))),
      db.update(authTokens).set({ usedAt: now }).where(and(eq(authTokens.userId, member.id), isNull(authTokens.usedAt))),
      db.insert(auditLogs).values({
        id: crypto.randomUUID(),
        actorUserId: actor.id,
        action: "organization.member_deactivated",
        subjectType: "user",
        subjectId: member.id,
        beforeJson: JSON.stringify({ status: member.status }),
        afterJson: JSON.stringify({ status: "frozen", deactivatedAt: now }),
        ipAddress: request.headers.get("cf-connecting-ip"),
        userAgent: request.headers.get("user-agent"),
      }),
    ]);

    return Response.json({ ok: true, memberId: member.id, email: member.email, status: "frozen", message: "成员账号已停用，历史记录已保留" });
  } catch (error) {
    return responseError(error);
  }
}
