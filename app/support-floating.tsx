...
export default function SupportFloating({ lang, telegramUrl }: { lang: Lang; telegramUrl?: string }) {
  const [open, setOpen] = useState(false);
  const zh = lang === "zh-CN" || lang === "zh-TW";
  const openTelegram = () => {
    const target = telegramUrl?.trim();
    if (target) {
  ...
  return <div className={`support-floating ${open ? "open" : ""}`}>
    <button className="support-fab" onClick={openTelegram} aria-label={zh ? "联系客户" : "Contact client"}>
      <img src="/support-agent-avatar.png" alt="" />
      <span>{zh ? "联系客户" : "Contact client"}</span>
    </button>
    {open && <section className="support-popover">
      ...
      {!telegramUrl?.trim() && <div className="support-telegram-unconfigured">{zh ? "Telegram 客服链接尚未配置，请在运维后台的系统配置中填写。" : "The Telegram support link is not configured yet. Add it in Maintenance → System configuration."}</div>}
