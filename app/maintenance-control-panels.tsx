...
export function MaintenanceSystemPanel() {
  ...
  return <div className="maintenance-content-grid">
    ...
    <section className="maintenance-panel"><header><div><span className="eyebrow">LIVE PREVIEW</span><h2>配置预览</h2></div><Badge tone="muted">保存后生效</Badge></header><div className="maintenance-brand-preview"><i>A</i><div><b>{value.siteName || "AgentNovas"}</b><span>{value.primaryDomain || "未配置域名"}</span><small>{value.maintenanceBanner || "当前没有维护公告"}</small></div></div><dl className="maintenance-definition-list"><div><dt>默认语言</dt><dd>{localeNames[value.defaultLocale] || value.defaultLocale}</dd></div><div><dt>启用语言数</dt><dd>{value.supportedLocales.length} 种</dd></div><div><dt>支持联系</dt><dd>{value.supportEmail}</dd></div></dl></section>
  </div>;
}
...
export function MaintenanceRolesPanel() {
  ...
  return <div className="maintenance-stack"><section className="maintenance-panel"><header><div><span className="eyebrow">ACCESS CONTROL</span><h2>五级运营权限矩阵</h2><p>权限由服务端角色与客户归属链共同控制；界面隐藏不能替代接口授权。</p></div><Badge>服务端强制</Badge></header><div className="maintenance-permission-matrix"><div className="head"><b>功能</b><span>员工</span><span>主管</span><span>经理</span><span>分公司</span><span>总公司</span></div>{rows.map((row) => <div key={row[0]}><b>{row[0]}</b>{row.slice(1).map((value, index) => <span key={`${row[0]}-${index}`}>{value}</span>)}</div>)}</div></section><section className="maintenance-panel"><header><div><span className="eyebrow">ROLE BOUNDARIES</span><h2>职能角色与技术权限</h2></div><Badge tone="muted">最小权限</Badge></header><div className="maintenance-rule-grid"><article><small>总公司技术 / 超级管理员</small><b>运维后台全部功能</b><span>目前运维入口暂挂在 hq_admin；后续应拆分独立技术角色</span></article><article><small>总公司客服</small><b>客户与公共池</b><span>不进入运维配置，不查看密钥</span></article><article><small>财务</small><b>账本与结算</b><span>不管理组织、不修改系统接口</span></article><article><small>审核员</small><b>策略审核与审计</b><span>不直接修改资金或执行配置</span></article></div></section></div>;
}
