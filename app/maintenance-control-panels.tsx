...
type ClientSettings = {
  system: { siteName: string; primaryDomain: string; supportEmail: string; telegramSupportUrl: string; copyrightOwner: string; defaultLocale: string; supportedLocales: string[]; maintenanceBanner: string };
...
const defaults: ClientSettings = {
  system: { siteName: "Riverton Capital", primaryDomain: "www.tzxsea.com", supportEmail: "support@agentnovas.com", telegramSupportUrl: "", copyrightOwner: "Riverton Capital", defaultLocale: "zh-CN", supportedLocales: allLocales, maintenanceBanner: "" },
...
export function MaintenanceSystemPanel() {
  ...
  return <div className="maintenance-content-grid">
    ...
    <section className="maintenance-panel"><header><div><span className="eyebrow">LIVE PREVIEW</span><h2>配置预览</h2></div><Badge tone="muted">编辑中预览</Badge></header><div className="maintenance-brand-preview"><i>R</i><div><b>{value.siteName || "Riverton Capital"}</b><span>{value.primaryDomain || "未配置域名"}</span><small>{value.maintenanceBanner || "当前没有维护公告"}</small></div></div><div className="maintenance-preview-location"><b>显示位置</b><span>用户网站顶部，紧接导航栏下方；保存后刷新页面即可看到。</span></div><dl className="maintenance-definition-list"><div><dt>默认语言</dt><dd>{localeNames[value.defaultLocale] || value.defaultLocale}</dd></div><div><dt>启用语言数</dt><dd>{value.supportedLocales.length} 种</dd></div><div><dt>支持联系</dt><dd>{value.supportEmail}</dd></div></dl></section>
  </div>;
}
