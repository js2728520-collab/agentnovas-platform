...
function StrategyReviewPanel() {
  ...
  return <section className="maintenance-panel maintenance-review-panel">
    ...
    <div className="maintenance-review-banner"><span className="maintenance-review-banner-icon">!</span><div><b>首要审核提醒</b><span>请优先处理策略广场的用户提交。当前策略采用双人审核，显示为“已通过人数 / 需要人数”。</span></div><StatusBadge tone={rows.length ? "warn" : "ok"}>{rows.length ? "需要处理" : "暂无待审核"}</StatusBadge></div>
    ...
    {status === "ready" && rows.length > 0 && <div className="maintenance-review-list">{rows.map((row) => {
      ...
      const required = Number(row.required || 2);
...
export function AdminGateway({ viewer, onSelect }: { viewer: AccountViewer | null; onSelect: (backend: AdminBackend) => void }) {
  const canOpenMaintenance = viewer?.role === "hq_admin";
...
export default function MaintenanceAdmin({ viewer, onBack }: { viewer: AccountViewer | null; onBack: () => void }) {
  ...
  return <div className="maintenance-admin">
    <header className="maintenance-head"><div><span className="eyebrow">PLATFORM MAINTENANCE</span><h1>运维后台</h1><p>给总公司技术人员使用 · 与运营后台共用数据库，功能和权限独立</p></div><div className="maintenance-head-actions"><StatusBadge>总公司技术权限</StatusBadge><button className="soft" onClick={onBack}>返回后台选择</button></div></header>
