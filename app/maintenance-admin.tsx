...
export function AdminGateway({ viewer, onSelect }: { viewer: AccountViewer | null; onSelect: (backend: AdminBackend) => void }) {
  ...
  return <div className="admin-gateway">
    ...
    <div className="admin-gateway-note"><i /> 当前账号：{(viewer?.nickname || viewer?.username || viewer?.email || "未登录").replace(/AgentNovas/gi, "Riverton Capital")} · 后台权限由服务端再次校验，卡片入口不会改变数据库权限。</div>
