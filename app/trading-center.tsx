...
export default function TradingCenter({ go }: TradingCenterProps) {
  ...
  const summary = (portfolio?.summary || {}) as Row;
  ...
  return <>
    ...
    <div className="trading-center-v2">
      ...
      <section className="trading-connections-v2"><header className="trading-section-head"><div><small>EXCHANGE CONNECTIONS</small><h2>交易所连接</h2></div><span>12 个连接入口</span></header><div className="trading-exchange-grid">{exchanges.map(([name, description]) => { const account = accounts.find(item => item.exchange === name); return <button className="trading-exchange" key={name} onClick={() => go("connect")}><ExchangeLogo name={name} /><span><b>{getExchangeDisplayName(name)}</b><small>{account ? `已绑定 · ${String(account.status)}` : description}</small></span><em className={account?.status === "active" ? "ok" : ""}>{account?.status === "active" ? "已启用" : account ? "待检测" : "连接"}</em></button>; })}</div><p className="trading-disclosure">连接仅用于保存凭证和检测权限；实盘订单路由尚未开放，保存凭证不会发送订单。客户策略模拟测试在“我的策略”中独立进行。</p></section>
