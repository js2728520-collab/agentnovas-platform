...
export default function TradingCenter({ go }: TradingCenterProps) {
  ...
  async function load() {
    const [accountResponse, orderResponse, portfolioResponse] = await Promise.all([
      fetch("/api/exchange-accounts", { cache: "no-store" }),
      fetch("/api/simulated-orders", { cache: "no-store" }),
      fetch("/api/portfolio", { cache: "no-store" }),
    ]);
    if (accountResponse.ok) setAccounts(((await accountResponse.json()).accounts || []) as Row[]);
    if (orderResponse.ok) setOrders(((await orderResponse.json()).orders || []) as Row[]);
  ...
  async function stopFollowing(subscriptionId: string) {
    setMessage("");
    const response = await fetch(`/api/strategy-subscriptions/${subscriptionId}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "stop" }) });
  ...
  return <>
    ...
    <div className="trading-center-v2">
      ...
      <section className="trading-connections-v2"><header className="trading-section-head"><div><small>EXCHANGE CONNECTIONS</small><h2>交易所连接</h2></div><span>12 个连接入口</span></header><div className="trading-exchange-grid">{exchanges.map(([name, description]) => { const account = accounts.find(item => item.exchange === name); return <button className="trading-exchange" key={name} onClick={() => go("connect")}><ExchangeLogo name={name} /><span><b>{getExchangeDisplayName(name)}</b><small>{account ? `${account.environment === "demo" ? "模拟盘" : "实盘"} · ${account.status}` : description}</small></span><em className={account?.status === "active" ? "ok" : ""}>{account?.status === "active" ? "已启用" : account ? "待检测" : "连接"}</em></button>; })}</div><p className="trading-disclosure">新增加密货币连接入口支持本地 Demo 绑定；Crypto.com、MetaMask、Robinhood、HTX 的官方权限和订单路由仍需独立适配与沙盒验证，实盘不会被默认开启。</p></section>
