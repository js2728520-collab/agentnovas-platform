...
usage() { printf 'usage: %s --check|--apply /absolute/path/to/payment-secret-broker-bootstrap.answers\n' "$0" >&2;exit 64; }
[ "$#" -eq 2 ] || usage
action=$1
answer_file=$2
...
while IFS= read -r line || [ -n "$line" ];do
  ...
  case "$line" in
    ''|'#'*) continue ;;
    PAYMENT_SECRET_BROKER_DATABASE_URL=*)
      [ -z "$database_url" ] || { printf 'duplicate database URL\n' >&2;exit 65; }
      database_url=${line#*=} ;;
    PAYMENT_ALLOWED_CALLBACK_HOSTS=*)
      [ -z "$allowed_hosts" ] || { printf 'duplicate callback host allowlist\n' >&2;exit 65; }
      allowed_hosts=${line#*=} ;;
    *) printf 'unsupported answer-file key\n' >&2;exit 65 ;;
  esac
done < "$answer_file"
[[ "$database_url" =~ ^postgresql://agentnovas_payment_secret_broker:[^[:space:]@]+@[^[:space:]/]+(:[0-9]+)?/agentnovas([?][^[:space:]]*)?$ ]] \
  || { printf 'broker database URL is invalid or uses the wrong role/database\n' >&2;exit 65; }
[ -n "$allowed_hosts" ] && [[ "$allowed_hosts" != *[[:space:]]* ]] \
  || { printf 'callback host allowlist is required and cannot contain whitespace\n' >&2;exit 65; }
IFS=',' read -r -a callback_hosts <<< "$allowed_hosts"
[ "${#callback_hosts[@]}" -le 10 ] || { printf 'too many callback hosts\n' >&2;exit 65; }
for host in "${callback_hosts[@]}";do
  [[ "$host" =~ ^([A-Za-z0-9-]+\.)*agentnovas\.com$ ]] \
    || { printf 'callback host is outside the controlled AgentNovas domain\n' >&2;exit 65; }
done
command -v openssl >/dev/null 2>&1 || { printf 'openssl is required\n' >&2;exit 69; }

secret_dir=${RIVERTON_SECRET_DIR:-/etc/agentnovas-riverton}
managed_dir=${RIVERTON_PAYMENT_SECRET_DIR:-$secret_dir/payment-managed}
...
for file in "$secret_dir/client.env" "$secret_dir/maintenance.env";do
...
update_env_file() {
  ...
  while [ "$#" -gt 0 ];do update_keys[${#update_keys[@]}]=$1;update_values[${#update_values[@]}]=$2;shift 2;done
...
private_key="$secret_dir/payment-secret-broker-private.pem"
public_key="$secret_dir/payment-secret-broker-public.pem"
...
if [ "$(id -u)" -eq 0 ];then chown "0:$service_gid" "$private_key";chmod 0440 "$private_key"
else chown "$service_uid:$service_gid" "$private_key";chmod 0400 "$private_key";fi
chmod 0444 "$public_key"
key_id="payment-broker-$(openssl pkey -pubin -in "$public_key" -outform DER 2>/dev/null | openssl dgst -sha256 -r | awk '{print substr($1,1,20)}')"

mkdir -p "$managed_dir/versions";chown -R "$service_uid:$service_gid" "$managed_dir";chmod 0700 "$managed_dir" "$managed_dir/versions"
update_env_file "$secret_dir/client.env" \
...
update_env_file "$secret_dir/maintenance.env" \
  ...
  PAYMENT_PROVIDER_OUTBOUND_ENABLED false \
  PAYMENT_ALLOWED_CALLBACK_HOSTS "$allowed_hosts" \
  PAYMENT_SECRET_BROKER_KEY_ID "$key_id" \
  PAYMENT_SECRET_BROKER_PUBLIC_KEY_PATH /run/secrets/payment-secret-broker-public.pem \
  PAYMENT_SECRET_DIRECTORY /run/payment-secrets \
  UDUN_ADDRESS_REQUEST_COIN_FIELD mainCoinType

broker_env="$secret_dir/payment-secret-broker.env"
umask 077
tmp=$(mktemp "${broker_env}.tmp.XXXXXX")
{
  printf 'NODE_ENV=production\n'
  printf 'DATABASE_URL=%s\n' "$database_url"
  printf 'PAYMENT_SECRET_BROKER_ENABLED=true\n'
  printf 'PAYMENT_SECRET_BROKER_KEY_ID=%s\n' "$key_id"
  printf 'PAYMENT_SECRET_BROKER_PRIVATE_KEY_PATH=/run/secrets/payment-secret-broker-private.pem\n'
  printf 'PAYMENT_SECRET_DIRECTORY=/run/payment-secrets\n'
  printf 'PAYMENT_ALLOWED_CALLBACK_HOSTS=%s\n' "$allowed_hosts"
...
printf 'provider_tests=disabled\n'
printf 'provider_outbound=disabled\n'
