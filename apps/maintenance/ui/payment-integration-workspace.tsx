  ...
  async function submit(commandToRun: PaymentCommand) {
    ...
    try {
      ...
      const response = await fetch(`/api/maintenance/payment-providers/${encodeURIComponent(commandToRun.provider.id)}/${test ? "test" : configure ? "configuration" : "status"}`, {
        ...
        body: JSON.stringify(test ? {} : configure ? { mainCoinType, tokenCoinType, walletId: walletId || null } : { status: commandToRun.kind === "activate" ? "active" : "disabled" }),
      });
        ...
        : <div className="rc-card-list">{resource.data.providers.map((provider) => <article key={provider.id}>
          ...
          {canManage && provider.provider === "udun" && <footer className="rc-action-row">
            {provider.configuredStatus === "disabled" && <>
              ...
              <button className="rc-button" type="button" disabled={busy || !tokenCoinType.trim()} onClick={() => submitDirect(provider, "configure")}>{t("保存币种映射")}</button>
            </>}
            <button className="rc-button" type="button" disabled={busy} onClick={() => submitDirect(provider, "test")}>{t("连通测试")}</button>
            {provider.configuredStatus === "active"
              ...
              : <button className="rc-button rc-button-primary" type="button" disabled={busy} onClick={() => void submit({ provider, kind: "activate" })}>{t("启用充值")}</button>}
          </footer>}
        </article>)}</div>}
    </section>
