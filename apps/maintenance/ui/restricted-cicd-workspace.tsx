...
export function RestrictedCicdWorkspace({ currentUserId, permissions }: { currentUserId: string; permissions: WorkflowPermissions }) {
  ...
  return <section className="rc-panel" aria-labelledby="restricted-cicd-title">
    ...
    {(allowed("maint.releases.workflow.activation.request") || canRequestCommand || allowed("maint.releases.workflow.stop")) ? <div className="rc-form"><InlineAuditReasonField id="restricted-cicd-reason" value={reason} onChange={setReason} label="本轮操作原因" hint="至少 8 个字符；将进入不可变事实与审计日志。" /></div> : null}
    ...
    {allowed("maint.releases.workflow.activation.request") ? <details className="rc-card"><summary>请求 activation</summary><div className="rc-form">
      ...
      <div className="rc-action-row"><button className="rc-primary" type="button" disabled={busy || !releaseVersionId || !hasValidAuditReason(reason)} onClick={() => void execute({ path: "/api/maintenance/release-workflow/activations", key: `activation:${environment}:${releaseVersionId}`, body: { releaseVersionId, environment, reason: reason.trim(), expiresAt: futureIso(120) }, success: "Activation 请求已追加；必须由两名不同 checker 分别完成 security 与 release 审批。" })}>提交 activation 请求</button></div>
    </div></details> : null}
    ...
    {canRequestCommand ? <details className="rc-card"><summary>请求 {environment} {action}</summary><div className="rc-form rc-form-grid">
      ...
      <div className="rc-action-row rc-wide-field"><button className="rc-primary" type="button" disabled={busy || !releaseVersionId || !hasValidAuditReason(reason)} onClick={() => void execute({ path: `/api/maintenance/release-workflow/commands/${environment}?releaseVersionId=${encodeURIComponent(releaseVersionId)}`, key: `command:${environment}:${action}:${releaseVersionId}`, body: { environment, action, reason: reason.trim() }, success: `${environment} ${action} 请求已追加；执行器仍保持关闭，审批也不代表已经部署。` })}>提交命令请求</button></div>
    </div></details> : null}

    {allowed("maint.releases.workflow.stop") ? <div className="rc-action-row"><button className="rc-danger-button" type="button" disabled={busy || !hasValidAuditReason(reason)} onClick={() => void execute({ path: "/api/maintenance/release-workflow/stops", key: `stop:${environment}`, body: { environment, reason: reason.trim() }, success: `${environment} sticky stop 已请求；目标确认前状态会保持 stop pending。` })}>请求 {environment} sticky stop</button></div> : null}

    <h3>Activation 请求</h3>
    {data.activationRequests.length === 0 ? <EmptyState title="尚无 activation 请求" description="先提交与 G7、provider、target 和 trust 摘要绑定的请求。" /> : <div className="rc-card-grid">{data.activationRequests.map((item) => <article className="rc-card" key={item.id}><header><StatusBadge value={item.active ? "active" : "pending"} /><time>{formatDateTime(item.createdAt)}</time></header><h3>{item.environment} · {short(item.artifactManifestSha256)}</h3><p>release {item.releaseVersionId} · control bundle {item.controlBundleId}</p><details><summary>服务端 control binding</summary>{Object.entries(item.controlBinding).map(([name, digest]) => <p key={name}>{name} <code>{digest}</code></p>)}</details><p>{item.reason}</p><p>security: {item.securityDecision ?? "待审"} · release: {item.releaseDecision ?? "待审"}</p>
      {allowed("maint.releases.workflow.activation.approve") && item.requestedByUserId !== currentUserId ? <div className="rc-action-row">{(["security", "release"] as const).map((kind) => <button key={kind} className="rc-button" type="button" disabled={busy || !hasValidAuditReason(reason) || (kind === "security" ? Boolean(item.securityDecision) : Boolean(item.releaseDecision))} onClick={() => void execute({ path: `/api/maintenance/release-workflow/activations/${encodeURIComponent(item.id)}/review`, key: `activation-review:${item.id}:${kind}`, body: { approvalKind: kind, decision: "approve", reason: reason.trim() }, success: `${kind} activation 审批已追加。` })}>批准 {kind}</button>)}</div> : null}
      {item.active && item.environment === "production" && allowed("maint.releases.workflow.production.enable") ? <div className="rc-action-row"><button className="rc-primary" type="button" disabled={busy || !hasValidAuditReason(reason)} onClick={() => void execute({ path: `/api/maintenance/release-workflow/activations/${encodeURIComponent(item.id)}/production-enablement`, key: `production-enable:${item.id}`, body: { reason: reason.trim(), expiresAt: futureIso(60) }, success: "首次 production enablement 已创建；仍需 G7 和显式命令审批。" })}>创建首次 production enablement</button></div> : null}
    </article>)}</div>}
