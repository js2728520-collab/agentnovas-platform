...
export function SystemHealthWorkspace({ overview = false }: { overview?: boolean }) {
  ...
  return <>
    ...
    <section className="rc-panel"><header><div><small>外部服务</small><h2>集成配置概况</h2></div></header><div className="rc-health-grid"><article><span>邮件服务</span><StatusBadge value={emailData ? emailData.configured ? "configured" : "unconfigured" : maintenanceResourceDisplayStatus(emailPhase)} /><small>{emailData ? `enabled ${emailData.workerEnabled ? "yes" : "no"} · send authorized ${emailData.sendAuthorized ? "yes" : "no"}` : resourceDetail(emailPhase, email.error)}</small></article><article><span>Beta 有效支付渠道</span><b>{paymentsData ? paymentsData.providers.filter((provider) => provider.effectiveStatus !== "disabled").length : resourceDisplayLabel(paymentsPhase)}</b><small>{paymentsData ? "Beta 策略要求为 0" : resourceDetail(paymentsPhase, payments.error)}</small></article><article><span>含密钥支付配置</span><b>{paymentsData ? paymentsData.providers.filter((provider) => provider.hasSecret).length : resourceDisplayLabel(paymentsPhase)}</b><small>{paymentsData ? "仅显示存在状态，不回显密钥" : resourceDetail(paymentsPhase, payments.error)}</small></article></div></section>
