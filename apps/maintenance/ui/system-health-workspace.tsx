...
type PlatformHealth = { status: string; mode: string; checks: Record<string, string | number | boolean>; timestamp: string };
type WorkerHealth = { paymentWorker: { enabled: boolean; configured: boolean }; notificationWorker: { enabled: boolean; resendConfigured: boolean } };
...
export function SystemHealthWorkspace({ overview = false }: { overview?: boolean }) {
  ...
  return <>
    ...
    <section className="rc-kpi-grid">
      <HealthCard label="数据库" value={String(health.data?.checks.database ?? "未知")} detail={health.data ? `检测于 ${formatDateTime(health.data.timestamp)}` : "未检测"} />
      <HealthCard label="Payment Worker" value={workers.data?.paymentWorker.enabled ? "开关已启用" : "开关已关闭"} detail={workers.data?.paymentWorker.configured ? "数据库配置存在" : "运行依赖未配置"} />
      <HealthCard label="Notification Worker" value={workers.data?.notificationWorker.enabled ? "开关已启用" : "开关已关闭"} detail={workers.data?.notificationWorker.resendConfigured ? "Resend 配置存在" : "Resend 未配置"} />
      <HealthCard label="永续真实订单" value="始终禁用" detail={health.data?.mode || "shadow-paper-only"} />
    </section>
    <section className="rc-panel"><header><div><small>核心检查</small><h2>平台运行约束</h2></div><StatusBadge value={health.data?.status || "unknown"} /></header><div className="rc-health-grid">{health.data && Object.entries(health.data.checks).map(([key, value]) => <article key={key}><span>{key}</span><StatusBadge value={String(value)} /></article>)}</div></section>
