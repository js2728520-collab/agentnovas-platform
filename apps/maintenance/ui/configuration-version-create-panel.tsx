...
export function ConfigurationVersionCreatePanel({ busy, onCreate, report }: {
  ...
  return <section className="rc-panel">
    ...
    <div className="rc-form rc-form-grid">
      ...
      <label>配置 key<input spellCheck={false} value={key} readOnly={registeredFeatureFlag} onChange={(event) => setKey(event.target.value)} placeholder="prompt.research_system" /><small>{registeredFeatureFlag ? "注册族固定接入 Client 策略研究入口。" : "小写字母开头，可使用数字、点、下划线和连字符。"}</small></label>
      <label>Schema 版本<input type="number" min={1} max={1_000_000} value={registeredFeatureFlag ? targetedFeatureFlag ? 2 : 1 : schemaVersion} readOnly={registeredFeatureFlag} onChange={(event) => setSchemaVersion(Number(event.target.value))} /></label>
      ...
      <div className="rc-action-row rc-wide-field"><button className="rc-primary" type="button" disabled={busy || !validKey || !targetingValid || !hasValidAuditReason(reason)} onClick={() => void submit()}>{busy ? "正在创建…" : "直接创建草稿"}</button></div>
    </div>
  </section>;
}
