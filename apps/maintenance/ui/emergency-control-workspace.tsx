...
export function EmergencyControlWorkspace() {
  ...
  async function execute(action: PendingAction) {
    ...
    try {
      const response = await fetch("/api/maintenance/trading/emergency-stop", {
        ...
        body: JSON.stringify({ active: action === "pause", incidentNote: incidentNote.trim() }),
      });
  ...
  return <>
    <PageHeading eyebrow="PAPER SAFETY CONTROL" title={t("紧急暂停")} description={t("按当前 RBAC 数据范围暂停官方 Paper 新开仓。操作者、范围和结果由服务端自动审计；事故说明属于业务处置记录。")} actions={<button className="rc-button" type="button" onClick={() => void resource.refresh()}>{t("刷新")}</button>} />
    ...
    <section className="rc-panel">
      ...
      <dl className="rc-description-list">
        ...
        <div><dt>{t("最近事故或处置说明")}</dt><dd>{state?.reason || t("尚无操作记录")}</dd></div>
      ...
      <div className="rc-form"><label>{t("事故或处置说明（业务字段）")}<textarea required minLength={3} maxLength={240} rows={3} value={incidentNote} onChange={(event) => setIncidentNote(event.target.value)} /></label><p className="rc-muted">{state?.active ? t("解除后组合不会自动恢复 active；仍需由会员或客户状态流程核验资格。") : t("暂停只会将范围内 active 的官方 Paper 组合改为仅允许平仓或只读，不发送订单。")}</p></div>
      <div className="rc-action-row">
        {state?.active
          ? <button className="rc-primary" type="button" disabled={busy || incidentNote.trim().length < 3} onClick={() => void execute("resume")}>{t("解除紧急暂停")}</button>
          : <button className="rc-button rc-danger-button" type="button" disabled={busy || incidentNote.trim().length < 3} onClick={() => void execute("pause")}>{t("暂停官方 Paper 新开仓")}</button>}
      </div>
    </section>
  </>;
}
