...
export function IntegrationsOverview({
  ...
  return <><PageHeading eyebrow="INTEGRATIONS" title="服务集成" description="安全查看邮件、优盾充值与 Demo 配置状态；配置存在不等于服务正在运行。" /><section className="rc-link-grid rc-integration-grid">{canViewEmail && <Link href="/integrations/email"><StatusBadge value={email.data?.configured ? "configured" : email.loading ? "loading" : "unconfigured"} /><small>Resend 邮件 · {email.data?.apiKeyPresent ? "密钥存在" : "密钥缺失"}</small></Link>}{canViewPayments && <Link href="/integrations/payments"><StatusBadge value={activePayments ? "active" : payments.loading ? "loading" : "disabled"} /><small>优盾充值 · {activePayments ?? "—"} 个有效网络</small></Link>}{canViewDemo && <Link href="/integrations/demo-exchanges"><StatusBadge value="safe view" /><small>Demo 现货账户、验证与停控</small></Link>}</section>{(email.error || payments.error) && <div className="rc-inline-error" role="alert">{email.error || payments.error}</div>}</>;
}
