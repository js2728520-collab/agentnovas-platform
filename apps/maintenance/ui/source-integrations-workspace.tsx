...
export function SourceIntegrationsWorkspace({ canTest }: { canTest: boolean }) {
  ...
  const [pending, setPending] = useState<SourceIntegration | null>(null);
  ...
  async function test(reason: string) {
    ...
    try {
      const response = await fetch(`/api/maintenance/integrations/${encodeURIComponent(pending.id)}/test`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ reason }) });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(apiErrorMessage(payload, "集成测试未通过"));
      setMessage(`${pending.name} 安全测试通过；该结果只证明固定只读端点当前可用。`);
      setPending(null);
