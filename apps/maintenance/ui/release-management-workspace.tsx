...
function ReleaseManagementControl({ initial, currentUserId, canManage, canApprove, refresh }: {
  ...
  return <>
    ...
    {(canManage || canApprove) ? <section className="rc-panel"><header><div><small>INLINE AUDIT</small><h2>{t("本轮版本操作")}</h2><p>{t("登记、复核和部署证据都在页面内填写原因后直接执行；不可变记录、maker/checker、幂等和权限边界不变。")}</p></div></header><div className="rc-form"><InlineAuditReasonField id="release-action-reason" value={auditReason} onChange={setAuditReason} label={t("版本操作原因")} hint={t("浏览器只登记既有证据，不执行 SSH、迁移、切流、Git 或部署命令。")} /></div></section> : null}
    ...
    {canManage ? <section className="rc-panel">
      ...
      <div className="rc-form rc-form-grid">
        ...
        <div className="rc-action-row rc-wide-field"><button className="rc-primary" type="button" disabled={busy || !hasValidAuditReason(auditReason)} onClick={() => void execute({ kind: "register" }, auditReason.trim())}>{t("检查并登记")}</button></div>
      </div>
    </section> : null}
    ...
    {selected ? <section className="rc-panel">
      ...
      <dl className="rc-description-list">
        ...
        <div><dt>{t("登记原因")}</dt><dd>{selected.reason}</dd></div>
      </dl>
      ...
      {!selected.verification && canApprove && selected.createdByUserId !== currentUserId ? <div className="rc-form rc-form-grid">
        ...
        <div className="rc-action-row rc-wide-field"><button className="rc-button" type="button" disabled={busy || verificationEvidence.length !== 64 || !hasValidAuditReason(auditReason)} onClick={() => void execute({ kind: "verify", release: selected, decision: "reject" }, auditReason.trim())}>{t("拒绝版本")}</button><button className="rc-primary" type="button" disabled={busy || verificationEvidence.length !== 64 || !hasValidAuditReason(auditReason)} onClick={() => void execute({ kind: "verify", release: selected, decision: "approve" }, auditReason.trim())}>{t("批准验证")}</button></div>
      </div> : null}
      {canApprove && selected.verification?.decision === "approve" ? <div className="rc-form rc-form-grid">
        ...
        <div className="rc-action-row rc-wide-field"><button className="rc-primary" type="button" disabled={busy || deploymentEvidence.length !== 64 || !hasValidAuditReason(auditReason)} onClick={() => void execute({ kind: "deployment", release: selected }, auditReason.trim())}>{t("登记已执行结果")}</button></div>
      </div> : null}
