...
type TreePayload = { rootId: string; nodes: TreeNode[]; summary: { organizations: number; members: number; customers: number; active: number } };
type PendingAction =
  | { kind: "invite" }
  | { kind: "relationship" }
...
export function OrganizationWorkspace({ canManage }: { canManage: boolean }) {
  ...
  return <>
    ...
    {canManage ? <section className="rc-panel"><header><div><small>MEMBER LIFECYCLE</small><h2>邀请与汇报关系</h2></div><StatusBadge value="一次性设置密码" /></header><div className="rc-card-grid">
      <article className="rc-card"><h3>邀请下一级成员</h3><div className="rc-form"><label>成员邮箱<input type="email" maxLength={254} value={invite.email} onChange={(event) => setInvite((current) => ({ ...current, email: event.target.value }))} /></label><label>分支名称（仅创建分支管理员时使用）<input maxLength={120} value={invite.name} onChange={(event) => setInvite((current) => ({ ...current, name: event.target.value }))} /></label><button className="rc-primary" type="button" disabled={busy || !invite.email.includes("@")} onClick={() => setPending({ kind: "invite" })}>发送设置密码邀请</button></div><p>不会生成或回显临时密码；邮件未送达前账户保持待激活。</p></article>
