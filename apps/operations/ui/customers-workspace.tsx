...
export function CustomersWorkspace() {
  ...
  return <>
    ...
    <ConfirmActionDialog open={Boolean(pendingAction)} title={t("确认客户操作")} description={`${t("操作将被审计：")}${pendingAction ?? ""}${t("。请确认目标客户和业务依据。")}`} confirmLabel={t("确认并记录")} reasonLabel={t("客户操作业务依据")} busy={busy} onCancel={() => setPendingAction(null)} onConfirm={(reason) => void submit(reason)} />
    <ConfirmActionDialog open={transferConfirming} title={t("提交客户归属调整")} description={t("申请只进入复核队列；另一名有权限的运营人员批准后才会生效。")} confirmLabel={t("确认提交")} reasonLabel={t("客户归属调整依据")} busy={busy} onCancel={() => setTransferConfirming(false)} onConfirm={(reason) => void submitTransfer(reason)} />
  </>;
}
