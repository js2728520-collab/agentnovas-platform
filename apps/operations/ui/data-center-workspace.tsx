...
export function DataCenterWorkspace() {
  ...
  return <>
    ...
    <section className="rc-panel"><header><div><small>PENDING QUEUE</small><h2>待处理队列</h2></div></header><div className="rc-link-grid"><Link href="/credits">{resource.data.pendingQueue.creditAdjustments}<small>Credits 调整</small></Link><Link href="/approvals">{resource.data.pendingQueue.attributionChanges}<small>归属调整</small></Link><Link href="/performance-statements">{resource.data.pendingQueue.performanceReviews}<small>绩效账单复核</small></Link></div></section>
  </>;
}
