...
function DepositList() {
  ...
  return <>
    ...
    <section className="rc-panel">
      <header><div><small>{t("只展示真实订单")}</small><h2>{t("充值查询")}</h2></div><div className="rc-filter-row"><label><span>{t("搜索")}</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("订单号、交易哈希或客户")} /></label><label><span>{t("状态")}</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">{t("全部")}</option><option>PENDING_CONFIRMATION</option><option>CONFIRMING</option><option>MANUAL_REVIEW</option><option>CREDITED</option><option>FAILED</option><option>RETURNED</option></select></label></div></header>
...
function DepositDetail({ depositId, canRequestAction }: { depositId: string; canRequestAction: boolean }) {
  ...
  return <>
    ...
    <ConfirmActionDialog open={confirmOpen} title={`${t("提交")} ${action}`} description={t("此操作必须由另一位有权限的运营人员审批。提交后不会自动改变资金或账本。")} confirmLabel={t("提交审批申请")} reasonLabel={t("人工资金操作依据")} busy={busy} onCancel={() => setConfirmOpen(false)} onConfirm={(reason) => void submit(reason)} />
  </>;
}
