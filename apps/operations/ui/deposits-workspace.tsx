...
function DepositDetail({ depositId, canRequestAction }: { depositId: string; canRequestAction: boolean }) {
  ...
  return <>
    <PageHeading eyebrow="DEPOSIT DETAIL" title={deposit.platformOrderNo} description="审批通过仅代表审批记录完成，不代表资金已执行。" actions={<Link className="rc-button" href="/deposits">返回列表</Link>} />
    ...
    {canRequestAction && <section className="rc-panel"><header><div><small>MAKER</small><h2>发起人工操作</h2></div></header><div className="rc-action-row"><label><span>操作类型</span><select value={action} onChange={(event) => setAction(event.target.value)}>{manualActions.map((value) => <option key={value}>{value}</option>)}</select></label><button className="rc-button" type="button" onClick={() => setConfirmOpen(true)}>填写原因并提交</button></div><p className="rc-muted">申请只进入审批队列，不会在浏览器或审批接口中执行资金变更。</p></section>}
