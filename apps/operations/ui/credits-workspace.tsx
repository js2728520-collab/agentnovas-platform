...
export function CreditsWorkspace({ canAdjust, canApprove }: { canAdjust: boolean; canApprove: boolean }) {
  ...
  return (
    <>
      ...
      <ConfirmActionDialog open={confirming && pending !== null} title={pending?.kind === "create" ? t("提交 Credits 调整申请") : `${pending?.decision === "approve" ? t("批准") : t("拒绝")} Credits ${t("调整")}`} description={pending?.kind === "create" ? t("请填写业务原因。提交后余额不会立即改变，必须由另一名有权限的复核人处理。") : t("复核结果会进入不可变 Credits 分录或以拒绝状态留档。")} confirmLabel={t("确认提交")} reasonLabel={t(pending?.kind === "create" ? "Credits 调整依据" : "复核业务说明")} busy={busy} onCancel={() => { setConfirming(false); if (pending?.kind === "decision") setPending(null); }} onConfirm={(reason) => void submitPending(reason)} />
    </>
  );
}
