...
export default function MembershipExperience() {
  ...
  async function createOrder() {
    if (!data || busy || !selectedCode || !acknowledged) return;
    ...
    } catch (error) { setMessage(clientErrorMessage(error, "会员申请创建失败")); }
    finally { setBusy(false); window.requestAnimationFrame(() => resultRef.current?.focus()); }
  }

  if (state === "loading") return <main className={styles.root} aria-busy="true" aria-label="会员中心加载中"><div className={styles.skeleton} /><div className={styles.skeleton} /></main>;
  if (state === "error" || !data) return <main className={styles.root}><section className={styles.panel}><h1>会员中心暂不可用</h1><p className={styles.error} role="alert">{message}</p><button className={styles.secondary} type="button" onClick={() => void load()}>重试</button></section></main>;
  ...
  return <main className={styles.root}>
    ...
    <section className={styles.panel} id="membership-payment" aria-labelledby="membership-application-title"><div className={styles.panelHead}><div><span className={styles.eyebrow}>APPLICATION</span><h2 id="membership-application-title">提交会员申请</h2></div>{selectedPlan && <span className={styles.badge}>{selectedPlan.name}</span>}</div><p className={styles.muted}>申请只会锁定服务端计划与法务快照，不代表付款成功或会员已开通。付款凭证需由 Operations 人工核验并双人审批。</p><div className={styles.legalList} aria-label="本次申请绑定的法务正文">{data.legalDocuments.map((document) => <article className={styles.legalItem} key={document.id}><details><summary><strong>{document.type}</strong><span>版本 {document.version} · {document.locale ?? "正文未配置"} · 生效 {formatDate(document.effectiveAt)}</span></summary>{document.contentMarkdown ? <div className={styles.notice}>{document.contentMarkdown}</div> : <p role="alert">当前版本没有可供阅读的已校验正文。</p>}</details><code title={document.contentSha256}>{document.contentSha256}</code></article>)}</div>{!data.orderCreationAvailable && <p className={styles.error} role="alert">七项法务正文尚未全部提供、校验并批准，当前不能创建付费申请。</p>}<label className={styles.acknowledgement} htmlFor={acknowledgementId}><input id={acknowledgementId} type="checkbox" checked={acknowledged} onChange={(event) => setAcknowledged(event.target.checked)} disabled={!data.orderCreationAvailable} /><span>我已逐项阅读以上七份正文，并同意本次申请绑定其版本与内容哈希；我理解提交申请不等于付款、激活或资金到账。</span></label><button className={styles.primary} type="button" disabled={busy || !selectedPlan || !acknowledged || !data.orderCreationAvailable} onClick={() => void createOrder()}>{busy ? "正在提交…" : "提交会员申请"}</button>{message && <div ref={resultRef} className={styles.notice} role="status" aria-live="polite" tabIndex={-1}>{message}</div>}</section>
    <section className={styles.panel} aria-labelledby="membership-orders-title"><div className={styles.panelHead}><div><span className={styles.eyebrow}>ORDER HISTORY</span><h2 id="membership-orders-title">申请记录</h2></div><span className={styles.badge}>{data.orders.length} 笔</span></div>{data.orders.length === 0 ? <p className={styles.notice}>暂无会员申请。提交后可在此追踪付款凭证、审核与激活状态。</p> : <div className={styles.orders}>{data.orders.map((order) => <article className={styles.order} key={order.id}><div><strong>{order.orderNo} · {order.plan.name}</strong><p>{order.plan.priceUsd} {order.plan.priceCurrency} · 创建于 {formatDate(order.createdAt)}</p></div><span className={styles.badge}>{statusLabels[order.status] ?? order.status}</span></article>)}</div>}</section>
  </main>;
}
