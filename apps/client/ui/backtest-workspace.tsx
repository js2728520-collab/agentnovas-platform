 ...
 * 这里把它接入策略中心：列表与详情都通过 /strategies 的 Tab 与查询参数到达。
 *
...
export default function BacktestWorkspace({ strategyId }: { strategyId?: string }) {
  ...
  if (strategyId) {
    return <StrategyBacktestDetail
      ...
      onBack={() => go("/strategies?tab=backtests")}
      onUpdated={() => void load()}
    />;
  }
  return <StrategyBacktestCenter
    strategies={strategies}
    onBack={() => go("/strategies?tab=research")}
    onOpenDetail={(id) => go(`/strategies?tab=backtests&strategy=${encodeURIComponent(id)}`)}
    onUpdated={() => void load()}
  />;
}
