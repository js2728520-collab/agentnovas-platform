...
export function NotificationWorkspace() {
  ...
  const load = useCallback(async () => {
    setState("loading"); setMessageKind("success");
    try {
  ...
  async function markRead(id?: string) {
    if (marking) return;
    setMarking(id ?? "all"); setMessage(""); setMessageKind("success");
    try {
      ...
      setMessageKind("success"); setMessage(id ? "通知已标记为已读" : "全部通知已标记为已读");
      await load();
    } catch (error) { setMessageKind("error"); setMessage(clientErrorMessage(error, "标记失败")); }
    finally { setMarking(""); }
  }
