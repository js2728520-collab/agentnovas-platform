...
export function LegalConsentExperience() {
  ...
  const resource = useApiData<CommercialLegalConsentStatus>("/api/membership/legal-consent", t("商业披露读取失败"));
  ...
  if (resource.loading && !documents) return <LoadingState label={t("正在读取商业披露…")} />;
  if (resource.error && !documents) return <ErrorState message={resource.error} retry={resource.refresh} />;
  if (!resource.data || !Array.isArray(documents)) return <ErrorState message={t("商业披露接口返回不完整，付费申请保持关闭。")} retry={resource.refresh} />;
  ...
  async function acceptLegalDocuments() {
    ...
    try {
      ...
      setResult({ kind: "success", message: t("当前七份正文的版本确认已保存；受邀账号的 3 天试用已在确认后由服务端开通。本次操作没有创建订单或付款。") });
      window.setTimeout(() => resultRef.current?.focus(), 0);
    } catch (error) {
      setResult({ kind: "error", message: clientErrorMessage(error, t("商业披露确认保存失败")) });
      window.setTimeout(() => resultRef.current?.focus(), 0);
    } finally {
      setBusy(false);
    }
  ...
  return <section className={styles.root} aria-label={t("商业披露与确认")}>
    <PageHeading
      eyebrow="CLIENT DISCLOSURE · VERSIONED CONTRACT"
      title={t("商业披露与版本确认")}
      description={t("逐项阅读当前有效版本。正文、版本与内容哈希来自服务端；披露未齐全时，系统会阻止付费申请。")}
      actions={<StatusBadge value={consentComplete ? t("确认已保存") : complete ? `7 / 7 ${t("已就绪")}` : `${documents.length} / 7 ${t("未齐全")}`} />}
    />

    <section className={styles.boundary} aria-labelledby="legal-boundary-title">
      <div>
        <span>{t("确认如何生效")}</span>
        <h2 id="legal-boundary-title">{t("先确认当前版本，再开通受邀试用")}</h2>
        <p>{t("当前七份正文的版本确认会独立保存；若账号有待开通的受邀试用，服务端会在同一流程中激活 3 天试用与三张 Paper 组合。不会创建付费订单或付款。")}</p>
      </div>
      {!consentComplete && complete ? <a className={styles.primaryLink} href="#legal-acceptance">{t("阅读后确认")}</a> : null}
    </section>

    {!complete && <div className={styles.blocked} role="alert">
      {t("商业披露发布尚未完成。缺少正文、完整性校验或双人复核时不能创建付费申请；平台不会以占位内容代替正式文本。")}
    </div>}
    <section className={styles.list} aria-label={t("当前生效的七项商业披露")}>
      {documents.length === 0 ? <div className={styles.empty}>{t("当前没有可阅读的有效商业披露，付费申请保持关闭。")}</div> : documents.map((document, index) =>
        <article className={styles.document} key={document.id}>
          <details open={index === 0}>
            <summary>
              <span className={styles.index}>{String(index + 1).padStart(2, "0")}</span>
              <span className={styles.name}>
                <strong>{t(documentLabels[document.type] ?? document.type)}</strong>
                <small>{t("版本")} {document.version} · {document.locale ?? t("语言未配置")} · {t("生效于")} {formatDateTime(document.effectiveAt, locale)}</small>
              </span>
              <span className={styles.readState}>{document.acceptedAt ? `${t("已确认")} ${formatDateTime(document.acceptedAt, locale)}` : document.contentMarkdown ? t("可阅读") : t("正文缺失")}</span>
            </summary>
            <div className={styles.body}>
              {document.contentMarkdown
                ? <LegalMarkdown source={document.contentMarkdown} />
                : <p role="alert">{t("当前版本没有可供阅读且已校验的正文。")}</p>}
              <dl className={styles.integrity}>
                <div><dt>{t("版本记录")}</dt><dd>{document.id}</dd></div>
                <div><dt>{t("内容哈希")}</dt><dd><code title={document.contentSha256}>{document.contentSha256}</code></dd></div>
              </dl>
            </div>
          </details>
        </article>)}
    ...
    {complete && !consentComplete && <section className={styles.acceptance} id="legal-acceptance" aria-labelledby="legal-acceptance-title">
      <div>
        ...
        <h2 id="legal-acceptance-title">{t("确认当前七份正文")}</h2>
        <p>{t("本次确认会保存正文 ID、版本、内容哈希、确认时间及请求环境信息；待开通试用会在确认后激活，但不会创建付费订单、付款记录或资金流水。")}</p>
      </div>
      <label className={styles.acknowledgement} htmlFor={acknowledgementId}>
        ...
        <span>{t("我已逐项阅读以上七份当前有效正文，并同意保存对应版本与内容哈希。")}</span>
      </label>
      <button className={styles.primaryLink} type="button" disabled={!acknowledged || busy} onClick={() => void acceptLegalDocuments()}>
        {busy ? t("正在保存确认…") : t("保存当前版本确认")}
      </button>
    </section>}
    {consentComplete && <section className={styles.completed} aria-label={t("商业披露确认已完成")}>
      <div><strong>{t("当前版本确认已完成")}</strong><span>{t("若任一正文发布新版本，系统会再次要求确认。")}</span></div>
      <Link className={styles.primaryLink} href={nextPath}>{nextPath === "/dashboard" ? t("进入数据看板") : t("继续访问原页面")}</Link>
    </section>}
