...
- [-] 4.5 策略回测、模拟盘和准入规则版本。
  ...
  - [!] 4.5b 模拟盘时长要求：P-05 确认**不设**时长门槛，把关改由人工审核承担，因此本项无需实现。
...
- [-] 4.7 策略浏览、筛选、卡片、详情、作者和风险披露。
  ...
  - [ ] 4.7b 筛选与排序（按品种、风险档、收益区间）；独立的单策略详情页（是否需要待定，广场卡片已覆盖历史表现）。
- [x] 4.8 订阅费、收益分成和作者/平台分账快照。跟单合同不可改写地固定策略版本、费率、分账比例与风险参数（INV-5）；分账走整数运算，平台份+作者份恒等于总额，尾差归作者。需求方确认：费率用客户会员档位、高水位线按 (客户,策略) 各自一条、**paper 不收费**。
- [-] 4.9 跟单账户、金额、仓位、止盈止损、杠杆、最大亏损。
  ...
  - [ ] 4.9c 止盈：现货跟单当前不设止盈，退出由策略 DSL 的 exit 条件驱动。
...
- [ ] 4.12 完成 G3 和策略市场浏览器验收。**未做**：策略广场与我的跟单两个新工作区没有跑过真实浏览器 E2E（四断点/键盘/axe），也没有跑 G3 门禁。单元与 PostgreSQL 层覆盖完整，但界面只有源码契约断言。
