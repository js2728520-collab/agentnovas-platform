...
- [-] 1.12 完成 G1、PostgreSQL 并发和多身份浏览器验收（本地关闭态 18/18、开启态三端登录预检 3/3；真实邮件与完整生产 MFA Gate 仍待）。
...
- [-] 3.1 通用配置 draft/test/approve/schedule/activate/rollback。
  ...
  - [-] 3.1b Maintenance 配置发布工作台、时区预览和最小权限到期激活器。
    - [x] 3.1b-UI `/configurations` 工作台、普通草稿/测试无弹窗、高风险操作确认、四断点/axe/真实 Chromium。
    - [ ] 3.1b-Worker 到期扫描、租约、最小数据库权限、幂等恢复和告警。
