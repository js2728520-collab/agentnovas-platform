...
- [!] P-11：正式域名冻结日。
- [!] P-12：目标验收日期和业务节点。
- [ ] 生成 Current → V3 代码/数据库/页面/route 详细矩阵。
- [ ] 用户评审并批准 Phase 1 开发。

## Phase 1：身份、权限与注册链接

- [ ] 1.1 从 Operations 导航和页面退休组织架构树/关系编辑。
- [ ] 1.2 保留并回归 assignment-bound 组织/分公司 scope。
- [ ] 1.3 新增角色权限注册链接迁移、约束和 token hash。
- [ ] 1.4 实现主管→员工、经理→主管/员工链接权限。
- [ ] 1.5 实现分公司总经理和总公司总经理链接权限。
- [ ] 1.6 实现重复使用、手动作废和重生成撤销旧链接。
- [ ] 1.7 实现自助注册原子创建身份、assignment、scope 和审计。
- [ ] 1.8 实现链接泄露响应、限流、会话排查和账号停用。
- [ ] 1.9 分离客户可复用邀请和内部角色 token。
- [ ] 1.10 Client 手机号/邮箱必填、邮箱验证、国际手机号。
- [ ] 1.11 Client 5 设备限制、新设备/异地通知和全量退出。
- [ ] 1.12 完成 G1、PostgreSQL 并发和四身份浏览器验收。
