...
- 客户 USDT 充值已按 ADR-0015 改为优盾 deposit-only：地址、验签和双审入账可用，目标商户未配置时失败关闭。提现、划转、自动扣款、自动退款、真实现货/永续交易和社区策略分账继续硬关闭。
...
- 独立的绩效账单、通知、账号安全、支持与公告页面；钱包账本保持只读，充值使用优盾真实订单和双审状态。
